...
// ??? sub_800BBE4(???);
// ??? HideAllUnits(???);
struct Unit* GetUnitStructFromEventParameter(short param);
...
// ??? sub_801550C(???);
// ??? UndeployEveryone(???);
void GotoChapterWithoutSave(u16 cid);
// ??? sub_80155C4(???);
...
void sub_8015D84(struct Proc* proc, s8 x, s8 y);
s8 EnsureCameraOntoPosition(struct Proc* proc, int x, int y);
