#include "gbafe.h"

CONST_DATA EventListScr EventScr_Ch12a_BeginningScene[] = {
    MUSC(SONG_47)
    LOAD2(0x1, UnitDef_Ch12AAlly_1)
    ENUN
    CUMO_CHAR(CHARACTER_EIRIKA)
    STAL(60)
    CURE
    TEXTSTART
    TEXTSHOW(0xa56)
    TEXTEND
    MUSCSLOW(SONG_SILENT)
    TEXTCONT
    TEXTEND
    REMA
    CAMERA2(10, 20)
    LOAD1(0x1, UnitDef_Ch12AEnemy_0)
    ENUN
    SVAL(EVT_SLOT_2, UnitDef_Ch12AEnemy_4)
    SVAL(EVT_SLOT_3, 0x1)
    CALL(EventScr_LoadUnitForTutorial)
    CUMO_CHAR(CHARACTER_SETH)
    STAL(60)
    CURE
    MUSC(SONG_TENSION)
    TEXTSTART
    TEXTSHOW(0xa57)
    TEXTEND
    REMA
    FADI(16)
    LOAD1(0x0, UnitDef_Event_Ch12aAlly)
    ENUN
    CALL(EventScr_CommonPrep)
    MUSCMID(SONG_SILENT)
    CAMERA2(7, 21)
    FADU(16)
    CUMO_CHAR(CHARACTER_SALEH)
    STAL(60)
    CURE
    MUSC(SONG_RISE_ABOVE)
    TEXTSTART
    TEXTSHOW(0xa58)
    TEXTEND
    REMA
    CAMERA2(9, 11)
    LOAD2(0x1, UnitDef_Ch12AMixed)
    ENUN
    CUMO_CHAR(CHARACTER_EWAN)
    STAL(60)
    CURE
    TEXTSTART
    TEXTSHOW(0xa59)
    TEXTEND
    REMA
    SVAL(EVT_SLOT_D, 0x0)
    SVAL(EVT_SLOT_1, 0x2c9)
    SENQUEUE1
    SVAL(EVT_SLOT_1, 0x0)
    SENQUEUE1
    SVAL(EVT_SLOT_1, 0x309)
    SENQUEUE1
    SVAL(EVT_SLOT_1, 0x0)
    SENQUEUE1
    SVAL(EVT_SLOT_1, 0x305)
    SENQUEUE1
    SVAL(EVT_SLOT_1, 0x0)
    SENQUEUE1
    MOVE_DEFINED(CHARACTER_EWAN)
    SVAL(EVT_SLOT_D, 0x0)
    SVAL(EVT_SLOT_1, 0x2cb)
    SENQUEUE1
    SVAL(EVT_SLOT_1, 0x0)
    SENQUEUE1
    SVAL(EVT_SLOT_1, 0x2c9)
    SENQUEUE1
    SVAL(EVT_SLOT_1, 0x0)
    SENQUEUE1
    SVAL(EVT_SLOT_1, 0x309)
    SENQUEUE1
    SVAL(EVT_SLOT_1, 0x0)
    SENQUEUE1
    SVAL(EVT_SLOT_1, 0x306)
    SENQUEUE1
    SVAL(EVT_SLOT_1, 0x0)
    SENQUEUE1
    MOVE_DEFINED(CHARACTER_FADO)
    ENUN
    DISA(CHARACTER_EWAN)
    STAL(30)
    MOVE(0x8, CHARACTER_FADO, 7, 12)
    ENUN
    CUMO_AT(5, 12)
    STAL(60)
    CURE
    SetBackground(BG_CAER_PELYN)
    MUSI
    TEXTSHOW(0xa5a)
    TEXTEND
    REMA
    MUNO
    FADI(16)
    CLEAN
    DISA(CHARACTER_FADO)
    LOAD1(0x1, UnitDef_Ch12AEnemy_1)
    ENUN
    ENUT(7)
    ENUT(8)
    EVBIT_T(7)
    ENDA
};

CONST_DATA EventListScr EventScr_Ch12a_EndingScene[] = {
    MUSC(SONG_VICTORY)
    FADI(16)
    CLEA
    CLEE
    CLEN
    CHECK_EXISTS(CHARACTER_EWAN)
    BNE(0x2, EVT_SLOT_C, EVT_SLOT_0)
    CHECK_ALIVE(CHARACTER_SALEH)
    BEQ(0x0, EVT_SLOT_C, EVT_SLOT_0)
    SetBackground(BG_CAER_PELYN)
    TEXTSHOW(0xa5b)
    TEXTEND
    GOTO(0x1)
LABEL(0x0)
    CHECK_ALIVE(CHARACTER_TETHYS)
    BEQ(0x2, EVT_SLOT_C, EVT_SLOT_0)
    SetBackground(BG_CAER_PELYN)
    TEXTSHOW(0xa5c)
    TEXTEND
LABEL(0x1)
    FADI(16)
    LOAD1(0x1, UnitDef_Ch12AAlly_0)
    ENUN
LABEL(0x2)
    CLEAN
    CAMERA2(12, 4)
    FADU(16)
    STAL(15)
    CUMO_AT(12, 1)
    STAL(60)
    CURE
    CHECK_ALIVE(CHARACTER_SALEH)
    BEQ(0xa, EVT_SLOT_C, EVT_SLOT_0)
    SetBackground(BG_VILLAGE_SUNSET)
    TEXTSHOW(0xa5d)
    TEXTEND
    REMA
    MUSCMID(SONG_SILENT)
    FADI(16)
    SetBackground(BG_CAER_PELYN)
    MUSC(SONG_LEGACY)
    TEXTSHOW(0xa5e)
    TEXTEND
    MUSCMID(SONG_SILENT)
    STAL3(32)
    MUSC(SONG_INTO_THE_SHADOW_OF_VICTORY)
    TEXTCONT
    TEXTEND
    REMA
    FADI(4)
    SetBackground(BG_VILLAGE_CLEAR)
    TEXTSHOW(0xa5f)
    TEXTEND
    REMA
    GOTO(0xb)
LABEL(0xa)
    MUSC(SONG_INTO_THE_SHADOW_OF_VICTORY)
    SetBackground(BG_VILLAGE_CLEAR)
    TEXTSHOW(0xa60)
    TEXTEND
    REMA
    MUSCMID(SONG_SILENT)
    FADI(16)
    SetBackground(BG_CAER_PELYN)
    MUSC(SONG_LEGACY)
    TEXTSHOW(0xa61)
    TEXTEND
    MUSCMID(SONG_SILENT)
    STAL3(32)
    MUSC(SONG_INTO_THE_SHADOW_OF_VICTORY)
    TEXTCONT
    TEXTEND
    REMA
    FADI(4)
    SetBackground(BG_VILLAGE_CLEAR)
    TEXTSHOW(0xa62)
    TEXTEND
    REMA
LABEL(0xb)
    ENUT(116)
    MNCH(0xd)
    ENDA
};

CONST_DATA EventListScr EventScr_Ch12A_0[] = {
    MUSI
    Text_BG(BG_CAER_PELYN, 0xa63)
    MUNO
    CALL(EventScr_RemoveBGIfNeeded)
    SVAL(EVT_SLOT_3, 0x59)
    GIVEITEMTO(CHAR_EVT_ACTIVE_UNIT)
    TILECHANGE(0x2)
    EVBIT_T(7)
    ENDA
};

CONST_DATA EventListScr EventScr_Ch12A_1[] = {
    MUSS(0x30)
    STAL(33)
LABEL(0x0)
    CHECK_ACTIVE
    SVAL(EVT_SLOT_7, 0x1)
    BEQ(0x1, EVT_SLOT_C, EVT_SLOT_7)
    SVAL(EVT_SLOT_7, 0x17)
    BEQ(0x2, EVT_SLOT_C, EVT_SLOT_7)
    SVAL(EVT_SLOT_7, 0x15)
    BEQ(0x3, EVT_SLOT_C, EVT_SLOT_7)
    Text_BG(BG_CAER_PELYN, 0xa67)
    GOTO(0x9)
LABEL(0x1)
    Text_BG(BG_CAER_PELYN, 0xa64)
    GOTO(0x9)
LABEL(0x2)
    Text_BG(BG_CAER_PELYN, 0xa65)
    GOTO(0x9)
LABEL(0x3)
    Text_BG(BG_CAER_PELYN, 0xa66)
LABEL(0x9)
    LOAD1(0x1, UnitDef_Ch12AAlly_0)
    ENUN
    TILECHANGE(0x3)
    MURE(0x2)
    EVBIT_T(7)
    ENDA
};

CONST_DATA EventListScr EventScr_Ch12A_2[] = {
    SVAL(EVT_SLOT_2, 0x0)
    CALL(EventScr_UnTriggerIfNotFaction)
    ENUF(7)
    EVBIT_T(7)
    ENDA
};

CONST_DATA EventListScr EventScr_Ch12A_3[] = {
    SVAL(EVT_SLOT_2, UnitDef_Ch12AEnemy_2)
    CALL(EventScr_LoadReinforce)
    SVAL(EVT_SLOT_2, UnitDef_Ch12AEnemy_7)
    CALL(EventScr_LoadReinforceHardMode)
    EVBIT_T(7)
    ENDA
};

CONST_DATA EventListScr EventScr_Ch12A_4[] = {
    SVAL(EVT_SLOT_2, 0x0)
    CALL(EventScr_UnTriggerIfNotFaction)
    ENUF(8)
    EVBIT_T(7)
    ENDA
};

CONST_DATA EventListScr EventScr_Ch12A_5[] = {
    SVAL(EVT_SLOT_2, UnitDef_Ch12AEnemy_5)
    CALL(EventScr_LoadReinforce)
    SVAL(EVT_SLOT_2, UnitDef_Ch12AEnemy_8)
    CALL(EventScr_LoadReinforceHardMode)
    SVAL(EVT_SLOT_2, UnitDef_Ch12AEnemy_6)
    CALL(EventScr_LoadReinforce)
    SVAL(EVT_SLOT_2, UnitDef_Ch12AEnemy_3)
    CALL(EventScr_LoadReinforce)
    EVBIT_T(7)
    ENDA
};
