...
struct ProcCmd CONST_DATA sProcScr_SALLYCURSORHelpPrompt[] =
{
...
void sub_80332D0(void)
{
    ...
    DisplayMoveRangeGraphics(LIMITVIEW_UNK);
    return;
}
...
//! FE8U = 0x08033548
void sub_8033548(struct ProcPrepSallyCursor * proc)
{
...
void PrepScreen_UnitSwapIdle(struct ProcPrepSallyCursor * proc)
{
    ...
    if (GetPlayerSelectKind(GetUnit(gBmMapUnit[gBmSt.playerCursor.y][gBmSt.playerCursor.x])) == 4)
    {
        r7 = 0;
    }
    ...
    AP_Update(proc->unk_54, gBmSt.playerCursorDisplay.x - gBmSt.camera.x, gBmSt.playerCursorDisplay.y - gBmSt.camera.y);
...
void sub_8034194(void)
{
    ...
    sub_80029E8(0x34, 0x100, 0x100, 0x18, NULL);
