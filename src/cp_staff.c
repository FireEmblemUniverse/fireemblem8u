...
extern struct AiStaffLutEntry gUnknown_080D868C[];

//! FE8U = 0x0803F9D8
int sub_803F9D8(u16 item) {
    u16 itemId;
    ...
    for (; gUnknown_080D868C[i].itemId != 0; i++) {
        if (itemId != gUnknown_080D868C[i].itemId) {
            continue;
        }
        
        if (gUnknown_080D868C[i].func != 0) {
            return i;
        }
    }
...
//! FE8U = 0x0803FA40
s8 sub_803FA40(s8 (*isEnemy)(struct Unit* unit)) {
    int i;
    ...
    for (i = 0; i < UNIT_ITEM_COUNT; i++) {
        ...
        funcIndex = sub_803F9D8(item);
        
        if (funcIndex == -1) {
            continue;
        }
        
        gUnknown_080D868C[funcIndex].func(i, isEnemy);
...
struct Unknown085A92BC {
    int x;
    int y;
};

extern struct Unknown085A92BC gUnknown_085A92BC[];

//! FE8U = 0x0803FAE8
s8 sub_803FAE8(int x, int y, struct Vec2* out) {
    int score;
    int ix;
    int iy;
    int i;
    ...
    for (i = 0; i < 4; i++) {
        ix = x + gUnknown_085A92BC[i].x;
        iy = y + gUnknown_085A92BC[i].y;
...
//! FE8U = 0x0803FBB8
void sub_803FBB8(int itemIdx, s8 (*isEnemy)(struct Unit* unit)) {
    int ix;
    int iy;
    struct Vec2 pos;
        ...
        for (ix = gBmMapSize.x - 1; ix >= 0; ix--) {
            ...
            if ((gAiState.flags & AI_FLAG_BERSERKED) || (isEnemy == 0) || (isEnemy(unit) != 1)) {
                ...
                if ((gAiState.unk7C != 0) || (unit->aiFlags & AI_UNIT_FLAG_0)) {
                    ...
                    if (tmpHpPerc <= lowestHpPerc && sub_803FAE8(ix, iy, &pos) != 0) {
...
//! FE8U = 0x0803FD30
void sub_803FD30(int itemIdx, s8 (*isEnemy)(struct Unit* unit)) {
    int i;
    struct Vec2 pos;
...
//! FE8U = 0x0803FF00
void sub_803FF00(int itemIdx,  s8 (*isEnemy)(struct Unit* unit)) {
    int ix;
    int iy;
    int tempUnk;
...
//! FE8U = 0x0804000C
void sub_804000C(int itemIdx,  s8 (*isEnemy)(struct Unit* unit)) {
    int ix;
    int iy;
    struct Vec2 out;
...
//! FE8U = 0x08040164
void sub_8040164(int itemIdx, s8 (*isEnemy)(struct Unit* unit)) {
    int ix;
    int iy;
    struct Vec2 pos;
...
//! FE8U = 0x08040300
u8 sub_8040300(struct Unit* unit) {
    u16 item;
...
//! FE8U = 0x08040354
void sub_8040354(int itemIdx, s8 (*isEnemy)(struct Unit* unit)) {
    u8 tmp;
    int i;
    struct Vec2 pos;
...
//! FE8U = 0x080404E0
void sub_80404E0(int itemIdx, s8 (*isEnemy)(struct Unit* unit)) {
    u8 accuracy;
    int i;
    struct Vec2 pos;
