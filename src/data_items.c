#include "global.h"
#include "bmitem.h"
#include "constants/items.h"

CONST_DATA struct ItemData gItemData[] = {
	[ITEM_NONE] = {
		.number = ITEM_NONE,
		.weaponType = ITYPE_SWORD,
		.iconId = 0x0,
	},
	[ITEM_SWORD_IRON] = {
		.nameTextId = 0x354,
		.descTextId = 0x404,
		.number = ITEM_SWORD_IRON,
		.weaponType = ITYPE_SWORD,
		.attributes = IA_WEAPON,
		.maxUses = 46,
		.might = 5,
		.hit = 90,
		.weight = 5,
		.encodedRange = 0x11,
		.costPerUse = 10,
		.weaponRank = WPN_EXP_E,
		.iconId = 0x0,
		.weaponExp = 1,
	},
	[ITEM_SWORD_SLIM] = {
		.nameTextId = 0x355,
		.descTextId = 0x405,
		.number = ITEM_SWORD_SLIM,
		.weaponType = ITYPE_SWORD,
		.attributes = IA_WEAPON,
		.maxUses = 30,
		.might = 3,
		.hit = 100,
		.weight = 2,
		.crit = 5,
		.encodedRange = 0x11,
		.costPerUse = 16,
		.weaponRank = WPN_EXP_E,
		.iconId = 0x1,
		.weaponExp = 1,
	},
	[ITEM_SWORD_STEEL] = {
		.nameTextId = 0x356,
		.descTextId = 0x406,
		.number = ITEM_SWORD_STEEL,
		.weaponType = ITYPE_SWORD,
		.attributes = IA_WEAPON,
		.maxUses = 30,
		.might = 8,
		.hit = 75,
		.weight = 10,
		.encodedRange = 0x11,
		.costPerUse = 20,
		.weaponRank = WPN_EXP_D,
		.iconId = 0x2,
		.weaponExp = 1,
	},
	[ITEM_SWORD_SILVER] = {
		.nameTextId = 0x357,
		.descTextId = 0x407,
		.number = ITEM_SWORD_SILVER,
		.weaponType = ITYPE_SWORD,
		.attributes = IA_WEAPON,
		.maxUses = 20,
		.might = 13,
		.hit = 80,
		.weight = 8,
		.encodedRange = 0x11,
		.costPerUse = 75,
		.weaponRank = WPN_EXP_A,
		.iconId = 0x3,
		.weaponExp = 1,
	},
	[ITEM_BLADE_IRON] = {
		.nameTextId = 0x358,
		.descTextId = 0x408,
		.number = ITEM_BLADE_IRON,
		.weaponType = ITYPE_SWORD,
		.attributes = IA_WEAPON,
		.maxUses = 35,
		.might = 9,
		.hit = 70,
		.weight = 12,
		.encodedRange = 0x11,
		.costPerUse = 28,
		.weaponRank = WPN_EXP_D,
		.iconId = 0x4,
		.weaponExp = 2,
	},
	[ITEM_BLADE_STEEL] = {
		.nameTextId = 0x359,
		.descTextId = 0x409,
		.number = ITEM_BLADE_STEEL,
		.weaponType = ITYPE_SWORD,
		.attributes = IA_WEAPON,
		.maxUses = 25,
		.might = 11,
		.hit = 65,
		.weight = 14,
		.encodedRange = 0x11,
		.costPerUse = 50,
		.weaponRank = WPN_EXP_C,
		.iconId = 0x5,
		.weaponExp = 2,
	},
	[ITEM_BLADE_SILVER] = {
		.nameTextId = 0x35a,
		.descTextId = 0x40a,
		.number = ITEM_BLADE_SILVER,
		.weaponType = ITYPE_SWORD,
		.attributes = IA_WEAPON,
		.maxUses = 15,
		.might = 14,
		.hit = 60,
		.weight = 13,
		.encodedRange = 0x11,
		.costPerUse = 120,
		.weaponRank = WPN_EXP_A,
		.iconId = 0x6,
		.weaponExp = 2,
	},
	[ITEM_SWORD_VENIN] = {
		.nameTextId = 0x35b,
		.descTextId = 0x40b,
		.number = ITEM_SWORD_VENIN,
		.weaponType = ITYPE_SWORD,
		.attributes = IA_WEAPON,
		.maxUses = 40,
		.might = 3,
		.hit = 70,
		.weight = 6,
		.encodedRange = 0x11,
		.weaponRank = WPN_EXP_D,
		.iconId = 0x7,
		.weaponEffectId = WPN_EFFECT_POISON,
		.weaponExp = 1,
	},
	[ITEM_SWORD_RAPIER] = {
		.nameTextId = 0x35c,
		.descTextId = 0x40c,
		.number = ITEM_SWORD_RAPIER,
		.weaponType = ITYPE_SWORD,
		.attributes = IA_WEAPON | IA_UNSELLABLE | IA_LOCK_4,
		.pEffectiveness = ItemEffectiveness_088ADEC2,
		.maxUses = 40,
		.might = 7,
		.hit = 95,
		.weight = 5,
		.crit = 10,
		.encodedRange = 0x11,
		.costPerUse = 150,
		.iconId = 0x8,
		.weaponExp = 2,
	},
	[ITEM_SWORD_MKATTI] = {
		.nameTextId = 0x403,
		.descTextId = 0x4ab,
		.number = ITEM_SWORD_MKATTI,
		.weaponType = ITYPE_SWORD,
		.attributes = IA_WEAPON | IA_UNSELLABLE | IA_LOCK_6,
		.pEffectiveness = ItemEffectiveness_088ADEC2,
		.maxUses = 45,
		.might = 8,
		.hit = 80,
		.weight = 3,
		.crit = 20,
		.encodedRange = 0x11,
		.iconId = 0x9,
		.weaponExp = 2,
	},
	[ITEM_SWORD_BRAVE] = {
		.nameTextId = 0x35d,
		.descTextId = 0x40d,
		.number = ITEM_SWORD_BRAVE,
		.weaponType = ITYPE_SWORD,
		.attributes = IA_WEAPON | IA_BRAVE,
		.maxUses = 30,
		.might = 9,
		.hit = 75,
		.weight = 12,
		.encodedRange = 0x11,
		.costPerUse = 250,
		.weaponRank = WPN_EXP_B,
		.iconId = 0xa,
		.weaponExp = 1,
	},
	[ITEM_SWORD_SHAMSIR] = {
		.nameTextId = 0x35e,
		.descTextId = 0x40e,
		.number = ITEM_SWORD_SHAMSIR,
		.weaponType = ITYPE_SWORD,
		.attributes = IA_WEAPON | IA_LOCK_2,
		.maxUses = 20,
		.might = 8,
		.hit = 75,
		.weight = 5,
		.crit = 35,
		.encodedRange = 0x11,
		.costPerUse = 60,
		.weaponRank = WPN_EXP_D,
		.iconId = 0xb2,
		.weaponExp = 1,
	},
	[ITEM_SWORD_KILLER] = {
		.nameTextId = 0x35f,
		.descTextId = 0x40f,
		.number = ITEM_SWORD_KILLER,
		.weaponType = ITYPE_SWORD,
		.attributes = IA_WEAPON,
		.maxUses = 20,
		.might = 9,
		.hit = 75,
		.weight = 7,
		.crit = 30,
		.encodedRange = 0x11,
		.costPerUse = 65,
		.weaponRank = WPN_EXP_C,
		.iconId = 0xc,
		.weaponExp = 1,
	},
	[ITEM_SWORD_ARMORSLAYER] = {
		.nameTextId = 0x360,
		.descTextId = 0x410,
		.number = ITEM_SWORD_ARMORSLAYER,
		.weaponType = ITYPE_SWORD,
		.attributes = IA_WEAPON,
		.pEffectiveness = ItemEffectiveness_088ADEBB,
		.maxUses = 18,
		.might = 8,
		.hit = 80,
		.weight = 11,
		.encodedRange = 0x11,
		.costPerUse = 70,
		.weaponRank = WPN_EXP_D,
		.iconId = 0xd,
		.weaponExp = 1,
	},
	[ITEM_SWORD_WYRMSLAYER] = {
		.nameTextId = 0x361,
		.descTextId = 0x411,
		.number = ITEM_SWORD_WYRMSLAYER,
		.weaponType = ITYPE_SWORD,
		.attributes = IA_WEAPON,
		.pEffectiveness = ItemEffectiveness_088ADF13,
		.maxUses = 30,
		.might = 8,
		.hit = 75,
		.weight = 5,
		.encodedRange = 0x11,
		.costPerUse = 150,
		.weaponRank = WPN_EXP_C,
		.iconId = 0xe,
		.weaponExp = 1,
	},
	[ITEM_SWORD_LIGHTBRAND] = {
		.nameTextId = 0x362,
		.descTextId = 0x412,
		.number = ITEM_SWORD_LIGHTBRAND,
		.weaponType = ITYPE_SWORD,
		.attributes = IA_WEAPON | IA_MAGICDAMAGE,
		.maxUses = 25,
		.might = 9,
		.hit = 70,
		.weight = 9,
		.encodedRange = 0x12,
		.costPerUse = 50,
		.weaponRank = WPN_EXP_C,
		.iconId = 0xf,
		.weaponExp = 1,
	},
	[ITEM_SWORD_RUNESWORD] = {
		.nameTextId = 0x363,
		.descTextId = 0x413,
		.number = ITEM_SWORD_RUNESWORD,
		.weaponType = ITYPE_SWORD,
		.attributes = IA_WEAPON | IA_MAGICDAMAGE,
		.maxUses = 15,
		.might = 12,
		.hit = 65,
		.weight = 11,
		.encodedRange = 0x12,
		.costPerUse = 220,
		.weaponRank = WPN_EXP_A,
		.iconId = 0x10,
		.weaponEffectId = WPN_EFFECT_HPDRAIN,
		.weaponExp = 1,
	},
	[ITEM_SWORD_LANCEREAVER] = {
		.nameTextId = 0x364,
		.descTextId = 0x414,
		.number = ITEM_SWORD_LANCEREAVER,
		.weaponType = ITYPE_SWORD,
		.attributes = IA_WEAPON | IA_REVERTTRIANGLE,
		.maxUses = 15,
		.might = 9,
		.hit = 75,
		.weight = 9,
		.crit = 5,
		.encodedRange = 0x11,
		.costPerUse = 120,
		.weaponRank = WPN_EXP_C,
		.iconId = 0x11,
		.weaponExp = 2,
	},
	[ITEM_SWORD_ZANBATO] = {
		.nameTextId = 0x365,
		.descTextId = 0x415,
		.number = ITEM_SWORD_ZANBATO,
		.weaponType = ITYPE_SWORD,
		.attributes = IA_WEAPON,
		.pEffectiveness = ItemEffectiveness_088ADEE0,
		.maxUses = 18,
		.might = 6,
		.hit = 85,
		.weight = 11,
		.encodedRange = 0x11,
		.costPerUse = 70,
		.weaponRank = WPN_EXP_D,
		.iconId = 0x12,
		.weaponExp = 1,
	},
	[ITEM_LANCE_IRON] = {
		.nameTextId = 0x369,
		.descTextId = 0x419,
		.number = ITEM_LANCE_IRON,
		.weaponType = ITYPE_LANCE,
		.attributes = IA_WEAPON,
		.maxUses = 45,
		.might = 7,
		.hit = 80,
		.weight = 8,
		.encodedRange = 0x11,
		.costPerUse = 8,
		.weaponRank = WPN_EXP_E,
		.iconId = 0x13,
		.weaponExp = 1,
	},
	[ITEM_LANCE_SLIM] = {
		.nameTextId = 0x36a,
		.descTextId = 0x41a,
		.number = ITEM_LANCE_SLIM,
		.weaponType = ITYPE_LANCE,
		.attributes = IA_WEAPON,
		.maxUses = 30,
		.might = 4,
		.hit = 85,
		.weight = 4,
		.crit = 5,
		.encodedRange = 0x11,
		.costPerUse = 15,
		.weaponRank = WPN_EXP_E,
		.iconId = 0x14,
		.weaponExp = 1,
	},
	[ITEM_LANCE_STEEL] = {
		.nameTextId = 0x36b,
		.descTextId = 0x41b,
		.number = ITEM_LANCE_STEEL,
		.weaponType = ITYPE_LANCE,
		.attributes = IA_WEAPON,
		.maxUses = 30,
		.might = 10,
		.hit = 70,
		.weight = 13,
		.encodedRange = 0x11,
		.costPerUse = 16,
		.weaponRank = WPN_EXP_D,
		.iconId = 0x15,
		.weaponExp = 2,
	},
	[ITEM_LANCE_SILVER] = {
		.nameTextId = 0x36c,
		.descTextId = 0x41c,
		.number = ITEM_LANCE_SILVER,
		.weaponType = ITYPE_LANCE,
		.attributes = IA_WEAPON,
		.maxUses = 20,
		.might = 14,
		.hit = 75,
		.weight = 10,
		.encodedRange = 0x11,
		.costPerUse = 60,
		.weaponRank = WPN_EXP_A,
		.iconId = 0x16,
		.weaponExp = 1,
	},
	[ITEM_LANCE_VENIN] = {
		.nameTextId = 0x36d,
		.descTextId = 0x41d,
		.number = ITEM_LANCE_VENIN,
		.weaponType = ITYPE_LANCE,
		.attributes = IA_WEAPON,
		.maxUses = 40,
		.might = 4,
		.hit = 65,
		.weight = 8,
		.encodedRange = 0x11,
		.weaponRank = WPN_EXP_E,
		.iconId = 0x17,
		.weaponEffectId = WPN_EFFECT_POISON,
		.weaponExp = 1,
	},
	[ITEM_LANCE_BRAVE] = {
		.nameTextId = 0x36e,
		.descTextId = 0x41e,
		.number = ITEM_LANCE_BRAVE,
		.weaponType = ITYPE_LANCE,
		.attributes = IA_WEAPON | IA_BRAVE,
		.maxUses = 30,
		.might = 10,
		.hit = 70,
		.weight = 14,
		.encodedRange = 0x11,
		.costPerUse = 250,
		.weaponRank = WPN_EXP_B,
		.iconId = 0x18,
		.weaponExp = 1,
	},
	[ITEM_LANCE_KILLER] = {
		.nameTextId = 0x36f,
		.descTextId = 0x41f,
		.number = ITEM_LANCE_KILLER,
		.weaponType = ITYPE_LANCE,
		.attributes = IA_WEAPON,
		.maxUses = 20,
		.might = 10,
		.hit = 70,
		.weight = 9,
		.crit = 30,
		.encodedRange = 0x11,
		.costPerUse = 60,
		.weaponRank = WPN_EXP_C,
		.iconId = 0x19,
		.weaponExp = 1,
	},
	[ITEM_LANCE_HORSESLAYER] = {
		.nameTextId = 0x370,
		.descTextId = 0x420,
		.number = ITEM_LANCE_HORSESLAYER,
		.weaponType = ITYPE_LANCE,
		.attributes = IA_WEAPON,
		.pEffectiveness = ItemEffectiveness_088ADEE0,
		.maxUses = 16,
		.might = 7,
		.hit = 70,
		.weight = 13,
		.encodedRange = 0x11,
		.costPerUse = 65,
		.weaponRank = WPN_EXP_D,
		.iconId = 0x1a,
		.weaponExp = 1,
	},
	[ITEM_LANCE_JAVELIN] = {
		.nameTextId = 0x371,
		.descTextId = 0x421,
		.number = ITEM_LANCE_JAVELIN,
		.weaponType = ITYPE_LANCE,
		.attributes = IA_WEAPON,
		.maxUses = 20,
		.might = 6,
		.hit = 65,
		.weight = 11,
		.encodedRange = 0x12,
		.costPerUse = 20,
		.weaponRank = WPN_EXP_E,
		.iconId = 0x1b,
		.weaponExp = 1,
	},
	[ITEM_LANCE_SPEAR] = {
		.nameTextId = 0x372,
		.descTextId = 0x422,
		.number = ITEM_LANCE_SPEAR,
		.weaponType = ITYPE_LANCE,
		.attributes = IA_WEAPON,
		.maxUses = 15,
		.might = 12,
		.hit = 70,
		.weight = 10,
		.crit = 5,
		.encodedRange = 0x12,
		.costPerUse = 600,
		.weaponRank = WPN_EXP_B,
		.iconId = 0x1c,
		.weaponExp = 1,
	},
	[ITEM_LANCE_AXEREAVER] = {
		.nameTextId = 0x373,
		.descTextId = 0x423,
		.number = ITEM_LANCE_AXEREAVER,
		.weaponType = ITYPE_LANCE,
		.attributes = IA_WEAPON | IA_REVERTTRIANGLE,
		.maxUses = 15,
		.might = 10,
		.hit = 70,
		.weight = 11,
		.crit = 5,
		.encodedRange = 0x11,
		.costPerUse = 130,
		.weaponRank = WPN_EXP_C,
		.iconId = 0x1d,
		.weaponExp = 2,
	},
	[ITEM_AXE_IRON] = {
		.nameTextId = 0x37a,
		.descTextId = 0x42a,
		.number = ITEM_AXE_IRON,
		.weaponType = ITYPE_AXE,
		.attributes = IA_WEAPON,
		.maxUses = 45,
		.might = 8,
		.hit = 75,
		.weight = 10,
		.encodedRange = 0x11,
		.costPerUse = 6,
		.weaponRank = WPN_EXP_E,
		.iconId = 0x1e,
		.weaponExp = 1,
	},
	[ITEM_AXE_STEEL] = {
		.nameTextId = 0x37b,
		.descTextId = 0x42b,
		.number = ITEM_AXE_STEEL,
		.weaponType = ITYPE_AXE,
		.attributes = IA_WEAPON,
		.maxUses = 30,
		.might = 11,
		.hit = 65,
		.weight = 15,
		.encodedRange = 0x11,
		.costPerUse = 12,
		.weaponRank = WPN_EXP_E,
		.iconId = 0x1f,
		.weaponExp = 2,
	},
	[ITEM_AXE_SILVER] = {
		.nameTextId = 0x37c,
		.descTextId = 0x42c,
		.number = ITEM_AXE_SILVER,
		.weaponType = ITYPE_AXE,
		.attributes = IA_WEAPON,
		.maxUses = 20,
		.might = 15,
		.hit = 70,
		.weight = 12,
		.encodedRange = 0x11,
		.costPerUse = 50,
		.weaponRank = WPN_EXP_A,
		.iconId = 0x20,
		.weaponExp = 1,
	},
	[ITEM_AXE_VENIN] = {
		.nameTextId = 0x37d,
		.descTextId = 0x42d,
		.number = ITEM_AXE_VENIN,
		.weaponType = ITYPE_AXE,
		.attributes = IA_WEAPON,
		.maxUses = 40,
		.might = 4,
		.hit = 60,
		.weight = 10,
		.encodedRange = 0x11,
		.weaponRank = WPN_EXP_D,
		.iconId = 0x21,
		.weaponEffectId = WPN_EFFECT_POISON,
		.weaponExp = 1,
	},
	[ITEM_AXE_BRAVE] = {
		.nameTextId = 0x37e,
		.descTextId = 0x42e,
		.number = ITEM_AXE_BRAVE,
		.weaponType = ITYPE_AXE,
		.attributes = IA_WEAPON | IA_BRAVE,
		.maxUses = 30,
		.might = 10,
		.hit = 65,
		.weight = 16,
		.encodedRange = 0x11,
		.costPerUse = 250,
		.weaponRank = WPN_EXP_B,
		.iconId = 0x22,
		.weaponExp = 1,
	},
	[ITEM_AXE_KILLER] = {
		.nameTextId = 0x37f,
		.descTextId = 0x42f,
		.number = ITEM_AXE_KILLER,
		.weaponType = ITYPE_AXE,
		.attributes = IA_WEAPON,
		.maxUses = 20,
		.might = 11,
		.hit = 65,
		.weight = 11,
		.crit = 30,
		.encodedRange = 0x11,
		.costPerUse = 50,
		.weaponRank = WPN_EXP_C,
		.iconId = 0x23,
		.weaponExp = 1,
	},
	[ITEM_AXE_HALBERD] = {
		.nameTextId = 0x380,
		.descTextId = 0x430,
		.number = ITEM_AXE_HALBERD,
		.weaponType = ITYPE_AXE,
		.attributes = IA_WEAPON,
		.pEffectiveness = ItemEffectiveness_088ADEE0,
		.maxUses = 18,
		.might = 10,
		.hit = 60,
		.weight = 15,
		.encodedRange = 0x11,
		.costPerUse = 45,
		.weaponRank = WPN_EXP_D,
		.iconId = 0x24,
		.weaponExp = 1,
	},
	[ITEM_AXE_HAMMER] = {
		.nameTextId = 0x381,
		.descTextId = 0x431,
		.number = ITEM_AXE_HAMMER,
		.weaponType = ITYPE_AXE,
		.attributes = IA_WEAPON,
		.pEffectiveness = ItemEffectiveness_088ADEBB,
		.maxUses = 20,
		.might = 10,
		.hit = 55,
		.weight = 15,
		.encodedRange = 0x11,
		.costPerUse = 40,
		.weaponRank = WPN_EXP_D,
		.iconId = 0x25,
		.weaponExp = 2,
	},
	[ITEM_AXE_DEVIL] = {
		.nameTextId = 0x382,
		.descTextId = 0x432,
		.number = ITEM_AXE_DEVIL,
		.weaponType = ITYPE_AXE,
		.attributes = IA_WEAPON,
		.maxUses = 20,
		.might = 18,
		.hit = 55,
		.weight = 18,
		.encodedRange = 0x11,
		.costPerUse = 45,
		.weaponRank = WPN_EXP_E,
		.iconId = 0x26,
		.weaponEffectId = WPN_EFFECT_DEVIL,
		.weaponExp = 8,
	},
	[ITEM_AXE_HANDAXE] = {
		.nameTextId = 0x383,
		.descTextId = 0x433,
		.number = ITEM_AXE_HANDAXE,
		.weaponType = ITYPE_AXE,
		.attributes = IA_WEAPON,
		.maxUses = 20,
		.might = 7,
		.hit = 60,
		.weight = 12,
		.encodedRange = 0x12,
		.costPerUse = 15,
		.weaponRank = WPN_EXP_E,
		.iconId = 0x27,
		.weaponExp = 1,
	},
	[ITEM_AXE_TOMAHAWK] = {
		.nameTextId = 0x384,
		.descTextId = 0x434,
		.number = ITEM_AXE_TOMAHAWK,
		.weaponType = ITYPE_AXE,
		.attributes = IA_WEAPON,
		.maxUses = 15,
		.might = 13,
		.hit = 65,
		.weight = 14,
		.encodedRange = 0x12,
		.costPerUse = 200,
		.weaponRank = WPN_EXP_A,
		.iconId = 0x28,
		.weaponExp = 1,
	},
	[ITEM_AXE_SWORDREAVER] = {
		.nameTextId = 0x385,
		.descTextId = 0x435,
		.number = ITEM_AXE_SWORDREAVER,
		.weaponType = ITYPE_AXE,
		.attributes = IA_WEAPON | IA_REVERTTRIANGLE,
		.maxUses = 15,
		.might = 11,
		.hit = 65,
		.weight = 13,
		.crit = 5,
		.encodedRange = 0x11,
		.costPerUse = 140,
		.weaponRank = WPN_EXP_C,
		.iconId = 0x29,
		.weaponExp = 2,
	},
	[ITEM_AXE_SWORDSLAYER] = {
		.nameTextId = 0x386,
		.descTextId = 0x436,
		.number = ITEM_AXE_SWORDSLAYER,
		.weaponType = ITYPE_AXE,
		.attributes = IA_WEAPON | IA_REVERTTRIANGLE,
		.pEffectiveness = ItemEffectiveness_088ADED7,
		.maxUses = 20,
		.might = 11,
		.hit = 80,
		.weight = 13,
		.crit = 5,
		.encodedRange = 0x11,
		.costPerUse = 100,
		.weaponRank = WPN_EXP_C,
		.iconId = 0x2a,
		.weaponExp = 1,
	},
	[ITEM_AXE_HATCHET] = {
		.nameTextId = 0x38a,
		.descTextId = 0x43a,
		.number = ITEM_AXE_HATCHET,
		.weaponType = ITYPE_AXE,
		.attributes = IA_WEAPON,
		.maxUses = 50,
		.might = 4,
		.hit = 85,
		.weight = 5,
		.encodedRange = 0x12,
		.weaponRank = WPN_EXP_E,
		.iconId = 0xb9,
		.weaponExp = 1,
	},
	[ITEM_BOW_IRON] = {
		.nameTextId = 0x38b,
		.descTextId = 0x43b,
		.number = ITEM_BOW_IRON,
		.weaponType = ITYPE_BOW,
		.attributes = IA_WEAPON,
		.pEffectiveness = ItemEffectiveness_088ADF2A,
		.maxUses = 45,
		.might = 6,
		.hit = 85,
		.weight = 5,
		.encodedRange = 0x22,
		.costPerUse = 12,
		.weaponRank = WPN_EXP_E,
		.iconId = 0x2b,
		.weaponExp = 1,
	},
	[ITEM_BOW_STEEL] = {
		.nameTextId = 0x38c,
		.descTextId = 0x43c,
		.number = ITEM_BOW_STEEL,
		.weaponType = ITYPE_BOW,
		.attributes = IA_WEAPON,
		.pEffectiveness = ItemEffectiveness_088ADF2A,
		.maxUses = 30,
		.might = 9,
		.hit = 70,
		.weight = 9,
		.encodedRange = 0x22,
		.costPerUse = 24,
		.weaponRank = WPN_EXP_D,
		.iconId = 0x2c,
		.weaponExp = 1,
	},
	[ITEM_BOW_SILVER] = {
		.nameTextId = 0x38d,
		.descTextId = 0x43d,
		.number = ITEM_BOW_SILVER,
		.weaponType = ITYPE_BOW,
		.attributes = IA_WEAPON,
		.pEffectiveness = ItemEffectiveness_088ADF2A,
		.maxUses = 20,
		.might = 13,
		.hit = 75,
		.weight = 6,
		.encodedRange = 0x22,
		.costPerUse = 80,
		.weaponRank = WPN_EXP_A,
		.iconId = 0x2d,
		.weaponExp = 1,
	},
	[ITEM_BOW_VENIN] = {
		.nameTextId = 0x38e,
		.descTextId = 0x43e,
		.number = ITEM_BOW_VENIN,
		.weaponType = ITYPE_BOW,
		.attributes = IA_WEAPON,
		.pEffectiveness = ItemEffectiveness_088ADF2A,
		.maxUses = 40,
		.might = 4,
		.hit = 65,
		.weight = 5,
		.encodedRange = 0x22,
		.weaponRank = WPN_EXP_D,
		.iconId = 0x2e,
		.weaponEffectId = WPN_EFFECT_POISON,
		.weaponExp = 1,
	},
	[ITEM_BOW_KILLER] = {
		.nameTextId = 0x38f,
		.descTextId = 0x43f,
		.number = ITEM_BOW_KILLER,
		.weaponType = ITYPE_BOW,
		.attributes = IA_WEAPON,
		.pEffectiveness = ItemEffectiveness_088ADF2A,
		.maxUses = 20,
		.might = 9,
		.hit = 75,
		.weight = 7,
		.crit = 30,
		.encodedRange = 0x22,
		.costPerUse = 70,
		.weaponRank = WPN_EXP_C,
		.iconId = 0x2f,
		.weaponExp = 1,
	},
	[ITEM_BOW_BRAVE] = {
		.nameTextId = 0x390,
		.descTextId = 0x440,
		.number = ITEM_BOW_BRAVE,
		.weaponType = ITYPE_BOW,
		.attributes = IA_WEAPON | IA_BRAVE,
		.pEffectiveness = ItemEffectiveness_088ADF2A,
		.maxUses = 30,
		.might = 10,
		.hit = 70,
		.weight = 12,
		.encodedRange = 0x22,
		.costPerUse = 250,
		.weaponRank = WPN_EXP_B,
		.iconId = 0x30,
		.weaponExp = 1,
	},
	[ITEM_BOW_SHORTBOW] = {
		.nameTextId = 0x391,
		.descTextId = 0x441,
		.number = ITEM_BOW_SHORTBOW,
		.weaponType = ITYPE_BOW,
		.attributes = IA_WEAPON,
		.pEffectiveness = ItemEffectiveness_088ADF2A,
		.maxUses = 22,
		.might = 5,
		.hit = 85,
		.weight = 3,
		.crit = 10,
		.encodedRange = 0x22,
		.costPerUse = 80,
		.weaponRank = WPN_EXP_D,
		.iconId = 0x31,
		.weaponExp = 2,
	},
	[ITEM_BOW_LONGBOW] = {
		.nameTextId = 0x392,
		.descTextId = 0x442,
		.number = ITEM_BOW_LONGBOW,
		.weaponType = ITYPE_BOW,
		.attributes = IA_WEAPON,
		.pEffectiveness = ItemEffectiveness_088ADF2A,
		.maxUses = 20,
		.might = 5,
		.hit = 65,
		.weight = 10,
		.encodedRange = 0x23,
		.costPerUse = 100,
		.weaponRank = WPN_EXP_D,
		.iconId = 0x32,
		.weaponExp = 1,
	},
	[ITEM_BALLISTA_REGULAR] = {
		.nameTextId = 0x393,
		.descTextId = 0x443,
		.number = ITEM_BALLISTA_REGULAR,
		.weaponType = ITYPE_BOW,
		.attributes = IA_WEAPON | IA_UNCOUNTERABLE,
		.pEffectiveness = ItemEffectiveness_088ADF2A,
		.maxUses = 5,
		.might = 8,
		.hit = 60,
		.weight = 20,
		.encodedRange = 0x3A,
		.weaponRank = WPN_EXP_E,
		.iconId = 0x33,
		.weaponExp = 4,
	},
	[ITEM_BALLISTA_LONG] = {
		.nameTextId = 0x394,
		.descTextId = 0x444,
		.number = ITEM_BALLISTA_LONG,
		.weaponType = ITYPE_BOW,
		.attributes = IA_WEAPON | IA_UNCOUNTERABLE,
		.pEffectiveness = ItemEffectiveness_088ADF2A,
		.maxUses = 5,
		.might = 13,
		.hit = 60,
		.weight = 20,
		.encodedRange = 0x3F,
		.weaponRank = WPN_EXP_E,
		.iconId = 0x34,
		.weaponExp = 4,
	},
	[ITEM_BALLISTA_KILLER] = {
		.nameTextId = 0x395,
		.descTextId = 0x445,
		.number = ITEM_BALLISTA_KILLER,
		.weaponType = ITYPE_BOW,
		.attributes = IA_WEAPON | IA_UNCOUNTERABLE,
		.pEffectiveness = ItemEffectiveness_088ADF2A,
		.maxUses = 5,
		.might = 12,
		.hit = 65,
		.weight = 20,
		.crit = 10,
		.encodedRange = 0x3A,
		.weaponRank = WPN_EXP_E,
		.iconId = 0x35,
		.weaponExp = 4,
	},
	[ITEM_ANIMA_FIRE] = {
		.nameTextId = 0x3a9,
		.descTextId = 0x458,
		.number = ITEM_ANIMA_FIRE,
		.weaponType = ITYPE_ANIMA,
		.attributes = IA_WEAPON | IA_MAGIC,
		.maxUses = 40,
		.might = 5,
		.hit = 90,
		.weight = 4,
		.encodedRange = 0x12,
		.costPerUse = 14,
		.weaponRank = WPN_EXP_E,
		.iconId = 0x36,
		.weaponExp = 1,
	},
	[ITEM_ANIMA_THUNDER] = {
		.nameTextId = 0x3aa,
		.descTextId = 0x459,
		.number = ITEM_ANIMA_THUNDER,
		.weaponType = ITYPE_ANIMA,
		.attributes = IA_WEAPON | IA_MAGIC,
		.maxUses = 35,
		.might = 8,
		.hit = 80,
		.weight = 6,
		.crit = 5,
		.encodedRange = 0x12,
		.costPerUse = 20,
		.weaponRank = WPN_EXP_D,
		.iconId = 0x37,
		.weaponExp = 1,
	},
	[ITEM_ANIMA_ELFIRE] = {
		.nameTextId = 0x3ab,
		.descTextId = 0x45a,
		.number = ITEM_ANIMA_ELFIRE,
		.weaponType = ITYPE_ANIMA,
		.attributes = IA_WEAPON | IA_MAGIC,
		.maxUses = 30,
		.might = 10,
		.hit = 85,
		.weight = 10,
		.encodedRange = 0x12,
		.costPerUse = 40,
		.weaponRank = WPN_EXP_C,
		.iconId = 0x38,
		.weaponExp = 2,
	},
	[ITEM_ANIMA_BOLTING] = {
		.nameTextId = 0x3ad,
		.descTextId = 0x45c,
		.number = ITEM_ANIMA_BOLTING,
		.weaponType = ITYPE_ANIMA,
		.attributes = IA_WEAPON | IA_MAGIC | IA_UNCOUNTERABLE,
		.maxUses = 5,
		.might = 12,
		.hit = 60,
		.weight = 20,
		.encodedRange = 0x3A,
		.costPerUse = 500,
		.weaponRank = WPN_EXP_B,
		.iconId = 0x39,
		.weaponExp = 1,
	},
	[ITEM_ANIMA_FIMBULVETR] = {
		.nameTextId = 0x3ae,
		.descTextId = 0x45d,
		.number = ITEM_ANIMA_FIMBULVETR,
		.weaponType = ITYPE_ANIMA,
		.attributes = IA_WEAPON | IA_MAGIC,
		.maxUses = 20,
		.might = 13,
		.hit = 80,
		.weight = 12,
		.encodedRange = 0x12,
		.costPerUse = 300,
		.weaponRank = WPN_EXP_A,
		.iconId = 0x3a,
		.weaponExp = 1,
	},
	[ITEM_ANIMA_FORBLAZE] = {
		.nameTextId = 0x403,
		.descTextId = 0x4ab,
		.number = ITEM_ANIMA_FORBLAZE,
		.weaponType = ITYPE_ANIMA,
		.attributes = IA_WEAPON | IA_MAGIC,
		.maxUses = 20,
		.might = 14,
		.hit = 85,
		.weight = 11,
		.crit = 5,
		.encodedRange = 0x12,
		.weaponRank = WPN_EXP_S,
		.iconId = 0x3b,
		.weaponExp = 1,
	},
	[ITEM_ANIMA_EXCALIBUR] = {
		.nameTextId = 0x3af,
		.descTextId = 0x45e,
		.number = ITEM_ANIMA_EXCALIBUR,
		.weaponType = ITYPE_ANIMA,
		.attributes = IA_WEAPON | IA_MAGIC | IA_UNSELLABLE,
		.pStatBonuses = &gUnknown_088AEEC4,
		.pEffectiveness = ItemEffectiveness_088ADF39,
		.maxUses = 30,
		.might = 18,
		.hit = 90,
		.weight = 13,
		.crit = 10,
		.encodedRange = 0x12,
		.weaponRank = WPN_EXP_S,
		.iconId = 0x3c,
		.weaponExp = 1,
	},
	[ITEM_LIGHT_LIGHTNING] = {
		.nameTextId = 0x3b0,
		.descTextId = 0x45f,
		.number = ITEM_LIGHT_LIGHTNING,
		.weaponType = ITYPE_LIGHT,
		.attributes = IA_WEAPON | IA_MAGIC,
		.maxUses = 35,
		.might = 4,
		.hit = 95,
		.weight = 6,
		.crit = 5,
		.encodedRange = 0x12,
		.costPerUse = 18,
		.weaponRank = WPN_EXP_E,
		.iconId = 0x3d,
		.weaponExp = 1,
	},
	[ITEM_LIGHT_SHINE] = {
		.nameTextId = 0x3b1,
		.descTextId = 0x460,
		.number = ITEM_LIGHT_SHINE,
		.weaponType = ITYPE_LIGHT,
		.attributes = IA_WEAPON | IA_MAGIC,
		.maxUses = 30,
		.might = 6,
		.hit = 90,
		.weight = 8,
		.crit = 8,
		.encodedRange = 0x12,
		.costPerUse = 30,
		.weaponRank = WPN_EXP_D,
		.iconId = 0x3e,
		.weaponExp = 2,
	},
	[ITEM_LIGHT_DIVINE] = {
		.nameTextId = 0x3b2,
		.descTextId = 0x461,
		.number = ITEM_LIGHT_DIVINE,
		.weaponType = ITYPE_LIGHT,
		.attributes = IA_WEAPON | IA_MAGIC,
		.maxUses = 25,
		.might = 8,
		.hit = 85,
		.weight = 12,
		.crit = 10,
		.encodedRange = 0x12,
		.costPerUse = 100,
		.weaponRank = WPN_EXP_C,
		.iconId = 0x3f,
		.weaponExp = 1,
	},
	[ITEM_LIGHT_PURGE] = {
		.nameTextId = 0x3b3,
		.descTextId = 0x462,
		.number = ITEM_LIGHT_PURGE,
		.weaponType = ITYPE_LIGHT,
		.attributes = IA_WEAPON | IA_MAGIC | IA_UNCOUNTERABLE,
		.maxUses = 5,
		.might = 10,
		.hit = 75,
		.weight = 20,
		.crit = 5,
		.encodedRange = 0x3A,
		.costPerUse = 600,
		.weaponRank = WPN_EXP_B,
		.iconId = 0x40,
		.weaponExp = 3,
	},
	[ITEM_LIGHT_AURA] = {
		.nameTextId = 0x3b4,
		.descTextId = 0x463,
		.number = ITEM_LIGHT_AURA,
		.weaponType = ITYPE_LIGHT,
		.attributes = IA_WEAPON | IA_MAGIC,
		.maxUses = 20,
		.might = 12,
		.hit = 85,
		.weight = 15,
		.crit = 15,
		.encodedRange = 0x12,
		.costPerUse = 400,
		.weaponRank = WPN_EXP_A,
		.iconId = 0x41,
		.weaponExp = 1,
	},
	[ITEM_LIGHT_LUCE] = {
		.nameTextId = 0x403,
		.descTextId = 0x4ab,
		.number = ITEM_LIGHT_LUCE,
		.weaponType = ITYPE_LIGHT,
		.attributes = IA_WEAPON | IA_MAGIC,
		.pEffectiveness = ItemEffectiveness_088ADF13,
		.maxUses = 20,
		.might = 6,
		.hit = 85,
		.weight = 10,
		.encodedRange = 0x12,
		.costPerUse = 200,
		.weaponRank = WPN_EXP_B,
		.iconId = 0x42,
		.weaponExp = 3,
	},
	[ITEM_DARK_FLUX] = {
		.nameTextId = 0x3b6,
		.descTextId = 0x465,
		.number = ITEM_DARK_FLUX,
		.weaponType = ITYPE_DARK,
		.attributes = IA_WEAPON | IA_MAGIC,
		.maxUses = 45,
		.might = 7,
		.hit = 80,
		.weight = 8,
		.encodedRange = 0x12,
		.costPerUse = 20,
		.weaponRank = WPN_EXP_D,
		.iconId = 0x43,
		.weaponExp = 1,
	},
	[ITEM_DARK_LUNA] = {
		.nameTextId = 0x3b7,
		.descTextId = 0x466,
		.number = ITEM_DARK_LUNA,
		.weaponType = ITYPE_DARK,
		.attributes = IA_WEAPON | IA_MAGIC | IA_NEGATE_DEFENSE,
		.maxUses = 30,
		.hit = 50,
		.weight = 12,
		.crit = 10,
		.encodedRange = 0x12,
		.costPerUse = 140,
		.weaponRank = WPN_EXP_C,
		.iconId = 0x44,
		.weaponExp = 1,
	},
	[ITEM_DARK_NOSFERATU] = {
		.nameTextId = 0x3b8,
		.descTextId = 0x467,
		.number = ITEM_DARK_NOSFERATU,
		.weaponType = ITYPE_DARK,
		.attributes = IA_WEAPON | IA_MAGIC,
		.maxUses = 20,
		.might = 10,
		.hit = 70,
		.weight = 14,
		.encodedRange = 0x12,
		.costPerUse = 160,
		.weaponRank = WPN_EXP_C,
		.iconId = 0x45,
		.weaponEffectId = WPN_EFFECT_HPDRAIN,
		.weaponExp = 1,
	},
	[ITEM_DARK_ECLIPSE] = {
		.nameTextId = 0x3b9,
		.descTextId = 0x468,
		.number = ITEM_DARK_ECLIPSE,
		.weaponType = ITYPE_DARK,
		.attributes = IA_WEAPON | IA_MAGIC | IA_UNCOUNTERABLE,
		.maxUses = 5,
		.might = 255,
		.hit = 30,
		.weight = 12,
		.encodedRange = 0x3A,
		.costPerUse = 800,
		.weaponRank = WPN_EXP_B,
		.iconId = 0x46,
		.weaponEffectId = WPN_EFFECT_HPHALVE,
		.weaponExp = 8,
	},
	[ITEM_DARK_FENRIR] = {
		.nameTextId = 0x3ba,
		.descTextId = 0x469,
		.number = ITEM_DARK_FENRIR,
		.weaponType = ITYPE_DARK,
		.attributes = IA_WEAPON | IA_MAGIC,
		.maxUses = 20,
		.might = 15,
		.hit = 70,
		.weight = 18,
		.encodedRange = 0x12,
		.costPerUse = 450,
		.weaponRank = WPN_EXP_A,
		.iconId = 0x47,
		.weaponExp = 1,
	},
	[ITEM_DARK_GLEIPNIR] = {
		.nameTextId = 0x3bb,
		.descTextId = 0x46a,
		.number = ITEM_DARK_GLEIPNIR,
		.weaponType = ITYPE_DARK,
		.attributes = IA_WEAPON | IA_MAGIC | IA_UNSELLABLE,
		.pStatBonuses = (void *)0x088AEED0,
		.maxUses = 30,
		.might = 23,
		.hit = 80,
		.weight = 20,
		.encodedRange = 0x12,
		.weaponRank = WPN_EXP_S,
		.iconId = 0xbc,
		.weaponExp = 3,
	},
	[ITEM_STAFF_HEAL] = {
		.nameTextId = 0x3c2,
		.descTextId = 0x471,
		.useDescTextId = 0x4af,
		.number = ITEM_STAFF_HEAL,
		.weaponType = ITYPE_STAFF,
		.attributes = IA_STAFF,
		.maxUses = 30,
		.hit = 100,
		.weight = 2,
		.encodedRange = 0x11,
		.costPerUse = 20,
		.weaponRank = WPN_EXP_E,
		.iconId = 0x49,
		.useEffectId = 1,
		.weaponExp = 2,
	},
	[ITEM_STAFF_MEND] = {
		.nameTextId = 0x3c3,
		.descTextId = 0x472,
		.useDescTextId = 0x4b0,
		.number = ITEM_STAFF_MEND,
		.weaponType = ITYPE_STAFF,
		.attributes = IA_STAFF,
		.maxUses = 20,
		.hit = 100,
		.weight = 4,
		.encodedRange = 0x11,
		.costPerUse = 50,
		.weaponRank = WPN_EXP_D,
		.iconId = 0x4a,
		.useEffectId = 2,
		.weaponExp = 3,
	},
	[ITEM_STAFF_RECOVER] = {
		.nameTextId = 0x3c4,
		.descTextId = 0x473,
		.useDescTextId = 0x4b1,
		.number = ITEM_STAFF_RECOVER,
		.weaponType = ITYPE_STAFF,
		.attributes = IA_STAFF,
		.maxUses = 15,
		.hit = 100,
		.weight = 6,
		.encodedRange = 0x11,
		.costPerUse = 150,
		.weaponRank = WPN_EXP_C,
		.iconId = 0x4b,
		.useEffectId = 3,
		.weaponExp = 3,
	},
	[ITEM_STAFF_PHYSIC] = {
		.nameTextId = 0x3c5,
		.descTextId = 0x474,
		.useDescTextId = 0x4b2,
		.number = ITEM_STAFF_PHYSIC,
		.weaponType = ITYPE_STAFF,
		.attributes = IA_STAFF,
		.maxUses = 15,
		.hit = 100,
		.weight = 5,
		.encodedRange = 0x10,
		.costPerUse = 250,
		.weaponRank = WPN_EXP_B,
		.iconId = 0x4c,
		.useEffectId = 4,
		.weaponExp = 3,
	},
	[ITEM_STAFF_FORTIFY] = {
		.nameTextId = 0x3c6,
		.descTextId = 0x475,
		.useDescTextId = 0x4b3,
		.number = ITEM_STAFF_FORTIFY,
		.weaponType = ITYPE_STAFF,
		.attributes = IA_STAFF,
		.maxUses = 8,
		.hit = 100,
		.weight = 7,
		.encodedRange = 0x10,
		.costPerUse = 1000,
		.weaponRank = WPN_EXP_A,
		.iconId = 0x4d,
		.useEffectId = 5,
		.weaponExp = 5,
	},
	[ITEM_STAFF_RESTORE] = {
		.nameTextId = 0x3c7,
		.descTextId = 0x476,
		.useDescTextId = 0x4b4,
		.number = ITEM_STAFF_RESTORE,
		.weaponType = ITYPE_STAFF,
		.attributes = IA_STAFF,
		.maxUses = 10,
		.hit = 100,
		.weight = 4,
		.encodedRange = 0x11,
		.costPerUse = 200,
		.weaponRank = WPN_EXP_C,
		.iconId = 0x4e,
		.useEffectId = 6,
		.weaponExp = 3,
	},
	[ITEM_STAFF_SILENCE] = {
		.nameTextId = 0x3c8,
		.descTextId = 0x477,
		.useDescTextId = 0x4b5,
		.number = ITEM_STAFF_SILENCE,
		.weaponType = ITYPE_STAFF,
		.attributes = IA_STAFF,
		.maxUses = 3,
		.hit = 70,
		.weight = 7,
		.encodedRange = 0x10,
		.costPerUse = 400,
		.weaponRank = WPN_EXP_B,
		.iconId = 0x4f,
		.useEffectId = 7,
		.weaponExp = 5,
	},
	[ITEM_STAFF_SLEEP] = {
		.nameTextId = 0x3c9,
		.descTextId = 0x478,
		.useDescTextId = 0x4b6,
		.number = ITEM_STAFF_SLEEP,
		.weaponType = ITYPE_STAFF,
		.attributes = IA_STAFF,
		.maxUses = 3,
		.hit = 65,
		.weight = 8,
		.encodedRange = 0x10,
		.costPerUse = 500,
		.weaponRank = WPN_EXP_B,
		.iconId = 0x50,
		.useEffectId = 8,
		.weaponExp = 5,
	},
	[ITEM_STAFF_BERSERK] = {
		.nameTextId = 0x3ca,
		.descTextId = 0x479,
		.useDescTextId = 0x4b7,
		.number = ITEM_STAFF_BERSERK,
		.weaponType = ITYPE_STAFF,
		.attributes = IA_STAFF,
		.maxUses = 3,
		.hit = 60,
		.weight = 8,
		.encodedRange = 0x10,
		.costPerUse = 600,
		.weaponRank = WPN_EXP_B,
		.iconId = 0x51,
		.useEffectId = 9,
		.weaponExp = 5,
	},
	[ITEM_STAFF_WARP] = {
		.nameTextId = 0x3cb,
		.descTextId = 0x47a,
		.useDescTextId = 0x4b8,
		.number = ITEM_STAFF_WARP,
		.weaponType = ITYPE_STAFF,
		.attributes = IA_STAFF,
		.maxUses = 5,
		.hit = 100,
		.weight = 5,
		.encodedRange = 0x11,
		.costPerUse = 1500,
		.weaponRank = WPN_EXP_A,
		.iconId = 0x52,
		.useEffectId = 10,
		.weaponExp = 7,
	},
	[ITEM_STAFF_RESCUE] = {
		.nameTextId = 0x3cc,
		.descTextId = 0x47b,
		.useDescTextId = 0x4b9,
		.number = ITEM_STAFF_RESCUE,
		.weaponType = ITYPE_STAFF,
		.attributes = IA_STAFF,
		.maxUses = 3,
		.hit = 100,
		.weight = 6,
		.encodedRange = 0x10,
		.costPerUse = 600,
		.weaponRank = WPN_EXP_B,
		.iconId = 0x53,
		.useEffectId = 11,
		.weaponExp = 7,
	},
	[ITEM_STAFF_TORCH] = {
		.nameTextId = 0x3cd,
		.descTextId = 0x47c,
		.useDescTextId = 0x4ba,
		.number = ITEM_STAFF_TORCH,
		.weaponType = ITYPE_STAFF,
		.attributes = IA_STAFF,
		.maxUses = 10,
		.hit = 100,
		.weight = 5,
		.encodedRange = 0x10,
		.costPerUse = 100,
		.weaponRank = WPN_EXP_D,
		.iconId = 0x54,
		.useEffectId = 12,
		.weaponExp = 5,
	},
	[ITEM_STAFF_REPAIR] = {
		.nameTextId = 0x3ce,
		.descTextId = 0x47d,
		.useDescTextId = 0x4bb,
		.number = ITEM_STAFF_REPAIR,
		.weaponType = ITYPE_STAFF,
		.attributes = IA_STAFF | IA_HAMMERNE,
		.maxUses = 3,
		.hit = 100,
		.weight = 7,
		.encodedRange = 0x11,
		.costPerUse = 600,
		.weaponRank = WPN_EXP_C,
		.iconId = 0x55,
		.useEffectId = 13,
		.weaponExp = 8,
	},
	[ITEM_STAFF_UNLOCK] = {
		.nameTextId = 0x3cf,
		.descTextId = 0x47e,
		.useDescTextId = 0x4bc,
		.number = ITEM_STAFF_UNLOCK,
		.weaponType = ITYPE_STAFF,
		.attributes = IA_STAFF,
		.maxUses = 10,
		.hit = 100,
		.weight = 7,
		.encodedRange = 0x12,
		.costPerUse = 150,
		.weaponRank = WPN_EXP_D,
		.iconId = 0x56,
		.useEffectId = 14,
		.weaponExp = 5,
	},
	[ITEM_STAFF_BARRIER] = {
		.nameTextId = 0x3d0,
		.descTextId = 0x47f,
		.useDescTextId = 0x4bd,
		.number = ITEM_STAFF_BARRIER,
		.weaponType = ITYPE_STAFF,
		.attributes = IA_STAFF,
		.maxUses = 15,
		.hit = 100,
		.weight = 4,
		.encodedRange = 0x11,
		.costPerUse = 150,
		.weaponRank = WPN_EXP_C,
		.iconId = 0x57,
		.useEffectId = 15,
		.weaponExp = 4,
	},
	[ITEM_AXE_DRAGON] = {
		.nameTextId = 0x387,
		.descTextId = 0x437,
		.number = ITEM_AXE_DRAGON,
		.weaponType = ITYPE_AXE,
		.attributes = IA_WEAPON,
		.pEffectiveness = ItemEffectiveness_088ADF13,
		.maxUses = 30,
		.might = 12,
		.hit = 60,
		.weight = 11,
		.encodedRange = 0x11,
		.costPerUse = 150,
		.weaponRank = WPN_EXP_C,
		.iconId = 0x58,
		.weaponExp = 1,
	},
	[ITEM_BOOSTER_HP] = {
		.nameTextId = 0x3d4,
		.descTextId = 0x483,
		.useDescTextId = 0x4c1,
		.number = ITEM_BOOSTER_HP,
		.weaponType = ITYPE_ITEM,
		.pStatBonuses = (void *)0x088AEEDC,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 8000,
		.iconId = 0x59,
		.useEffectId = 16,
	},
	[ITEM_BOOSTER_POW] = {
		.nameTextId = 0x3d5,
		.descTextId = 0x484,
		.useDescTextId = 0x4c2,
		.number = ITEM_BOOSTER_POW,
		.weaponType = ITYPE_ITEM,
		.pStatBonuses = (void *)0x088AEEE8,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 8000,
		.iconId = 0x5a,
		.useEffectId = 17,
	},
	[ITEM_BOOSTER_SKL] = {
		.nameTextId = 0x3d6,
		.descTextId = 0x485,
		.useDescTextId = 0x4c3,
		.number = ITEM_BOOSTER_SKL,
		.weaponType = ITYPE_ITEM,
		.pStatBonuses = (void *)0x088AEEF4,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 8000,
		.iconId = 0x5b,
		.useEffectId = 18,
	},
	[ITEM_BOOSTER_SPD] = {
		.nameTextId = 0x3d7,
		.descTextId = 0x486,
		.useDescTextId = 0x4c4,
		.number = ITEM_BOOSTER_SPD,
		.weaponType = ITYPE_ITEM,
		.pStatBonuses = (void *)0x088AEF00,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 8000,
		.iconId = 0x5c,
		.useEffectId = 19,
	},
	[ITEM_BOOSTER_LCK] = {
		.nameTextId = 0x3d8,
		.descTextId = 0x487,
		.useDescTextId = 0x4c5,
		.number = ITEM_BOOSTER_LCK,
		.weaponType = ITYPE_ITEM,
		.pStatBonuses = (void *)0x088AEF0C,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 8000,
		.iconId = 0x5d,
		.useEffectId = 20,
	},
	[ITEM_BOOSTER_DEF] = {
		.nameTextId = 0x3d9,
		.descTextId = 0x488,
		.useDescTextId = 0x4c6,
		.number = ITEM_BOOSTER_DEF,
		.weaponType = ITYPE_ITEM,
		.pStatBonuses = (void *)0x088AEF18,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 8000,
		.iconId = 0x5e,
		.useEffectId = 21,
	},
	[ITEM_BOOSTER_RES] = {
		.nameTextId = 0x3da,
		.descTextId = 0x489,
		.useDescTextId = 0x4c7,
		.number = ITEM_BOOSTER_RES,
		.weaponType = ITYPE_ITEM,
		.pStatBonuses = (void *)0x088AEF24,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 8000,
		.iconId = 0x5f,
		.useEffectId = 22,
	},
	[ITEM_BOOSTER_MOV] = {
		.nameTextId = 0x3db,
		.descTextId = 0x48a,
		.useDescTextId = 0x4c8,
		.number = ITEM_BOOSTER_MOV,
		.weaponType = ITYPE_ITEM,
		.pStatBonuses = (void *)0x088AEF30,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 8000,
		.iconId = 0x60,
		.useEffectId = 23,
	},
	[ITEM_BOOSTER_CON] = {
		.nameTextId = 0x3dc,
		.descTextId = 0x48b,
		.useDescTextId = 0x4c9,
		.number = ITEM_BOOSTER_CON,
		.weaponType = ITYPE_ITEM,
		.pStatBonuses = (void *)0x088AEF3C,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 8000,
		.iconId = 0x61,
		.useEffectId = 24,
	},
	[ITEM_HEROCREST] = {
		.nameTextId = 0x3dd,
		.descTextId = 0x48c,
		.useDescTextId = 0x4ca,
		.number = ITEM_HEROCREST,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 10000,
		.iconId = 0x62,
		.useEffectId = 25,
	},
	[ITEM_KNIGHTCREST] = {
		.nameTextId = 0x3de,
		.descTextId = 0x48d,
		.useDescTextId = 0x4cb,
		.number = ITEM_KNIGHTCREST,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 10000,
		.iconId = 0x63,
		.useEffectId = 26,
	},
	[ITEM_ORIONSBOLT] = {
		.nameTextId = 0x3df,
		.descTextId = 0x48e,
		.useDescTextId = 0x4cc,
		.number = ITEM_ORIONSBOLT,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 10000,
		.iconId = 0x64,
		.useEffectId = 27,
	},
	[ITEM_ELYSIANWHIP] = {
		.nameTextId = 0x3e0,
		.descTextId = 0x48f,
		.useDescTextId = 0x4cd,
		.number = ITEM_ELYSIANWHIP,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 10000,
		.iconId = 0x65,
		.useEffectId = 28,
	},
	[ITEM_GUIDINGRING] = {
		.nameTextId = 0x3e1,
		.descTextId = 0x490,
		.useDescTextId = 0x4ce,
		.number = ITEM_GUIDINGRING,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 10000,
		.iconId = 0x66,
		.useEffectId = 29,
	},
	[ITEM_CHESTKEY] = {
		.nameTextId = 0x3e2,
		.descTextId = 0x491,
		.useDescTextId = 0x4cf,
		.number = ITEM_CHESTKEY,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 300,
		.iconId = 0x67,
		.useEffectId = 30,
	},
	[ITEM_DOORKEY] = {
		.nameTextId = 0x3e3,
		.descTextId = 0x492,
		.useDescTextId = 0x4d0,
		.number = ITEM_DOORKEY,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 50,
		.iconId = 0x68,
		.useEffectId = 31,
	},
	[ITEM_LOCKPICK] = {
		.nameTextId = 0x3e4,
		.descTextId = 0x493,
		.useDescTextId = 0x4d1,
		.number = ITEM_LOCKPICK,
		.weaponType = ITYPE_ITEM,
		.maxUses = 15,
		.encodedRange = 0x11,
		.costPerUse = 80,
		.iconId = 0x69,
		.useEffectId = 32,
	},
	[ITEM_VULNERARY] = {
		.nameTextId = 0x3e5,
		.descTextId = 0x494,
		.useDescTextId = 0x4d2,
		.number = ITEM_VULNERARY,
		.weaponType = ITYPE_ITEM,
		.maxUses = 3,
		.encodedRange = 0x11,
		.costPerUse = 100,
		.iconId = 0x6a,
		.useEffectId = 33,
	},
	[ITEM_ELIXIR] = {
		.nameTextId = 0x3e6,
		.descTextId = 0x495,
		.useDescTextId = 0x4d3,
		.number = ITEM_ELIXIR,
		.weaponType = ITYPE_ITEM,
		.maxUses = 3,
		.encodedRange = 0x11,
		.costPerUse = 1000,
		.iconId = 0x6b,
		.useEffectId = 34,
	},
	[ITEM_PUREWATER] = {
		.nameTextId = 0x3e7,
		.descTextId = 0x496,
		.useDescTextId = 0x4d4,
		.number = ITEM_PUREWATER,
		.weaponType = ITYPE_ITEM,
		.maxUses = 3,
		.encodedRange = 0x11,
		.costPerUse = 300,
		.iconId = 0x6c,
		.useEffectId = 35,
	},
	[ITEM_ANTITOXIN] = {
		.nameTextId = 0x3e8,
		.descTextId = 0x497,
		.useDescTextId = 0x4d5,
		.number = ITEM_ANTITOXIN,
		.weaponType = ITYPE_ITEM,
		.maxUses = 3,
		.encodedRange = 0x11,
		.costPerUse = 150,
		.iconId = 0x6d,
		.useEffectId = 36,
	},
	[ITEM_TORCH] = {
		.nameTextId = 0x3e9,
		.descTextId = 0x498,
		.useDescTextId = 0x4d6,
		.number = ITEM_TORCH,
		.weaponType = ITYPE_ITEM,
		.maxUses = 5,
		.encodedRange = 0x11,
		.costPerUse = 100,
		.iconId = 0x6e,
		.useEffectId = 37,
	},
	[ITEM_DELPHISHIELD] = {
		.nameTextId = 0x3ea,
		.descTextId = 0x499,
		.useDescTextId = 0x4d7,
		.number = ITEM_DELPHISHIELD,
		.weaponType = ITYPE_ITEM,
		.attributes = IA_UNBREAKABLE | IA_NEGATE_FLYING,
		.encodedRange = 0x11,
		.costPerUse = 10000,
		.iconId = 0xda,
	},
	[ITEM_MEMBERCARD] = {
		.nameTextId = 0x3eb,
		.descTextId = 0x49a,
		.useDescTextId = 0x4d8,
		.number = ITEM_MEMBERCARD,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 6000,
		.iconId = 0x84,
	},
	[ITEM_SILVERCARD] = {
		.nameTextId = 0x3ec,
		.descTextId = 0x49b,
		.useDescTextId = 0x4d9,
		.number = ITEM_SILVERCARD,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 4000,
		.iconId = 0x85,
	},
	[ITEM_WHITEGEM] = {
		.nameTextId = 0x3ef,
		.descTextId = 0x49e,
		.number = ITEM_WHITEGEM,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 20000,
		.iconId = 0x86,
	},
	[ITEM_BLUEGEM] = {
		.nameTextId = 0x3f0,
		.descTextId = 0x49f,
		.number = ITEM_BLUEGEM,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 10000,
		.iconId = 0x87,
	},
	[ITEM_REDGEM] = {
		.nameTextId = 0x3f1,
		.descTextId = 0x4a0,
		.number = ITEM_REDGEM,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 5000,
		.iconId = 0x88,
	},
	[ITEM_GOLD] = {
		.nameTextId = 0x3f2,
		.descTextId = 0x4a1,
		.useDescTextId = 0x4da,
		.number = ITEM_GOLD,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.iconId = 0x89,
	},
	[ITEM_LANCE_REGINLEIF] = {
		.nameTextId = 0x374,
		.descTextId = 0x424,
		.number = ITEM_LANCE_REGINLEIF,
		.weaponType = ITYPE_LANCE,
		.attributes = IA_WEAPON | IA_UNSELLABLE | IA_LOCK_5,
		.pEffectiveness = ItemEffectiveness_088ADEC2,
		.maxUses = 45,
		.might = 10,
		.hit = 80,
		.weight = 8,
		.crit = 10,
		.encodedRange = 0x11,
		.costPerUse = 160,
		.iconId = 0xb3,
		.weaponExp = 2,
	},
	[ITEM_CHESTKEY_BUNDLE] = {
		.nameTextId = 0x3e2,
		.descTextId = 0x491,
		.useDescTextId = 0x4cf,
		.number = ITEM_CHESTKEY_BUNDLE,
		.weaponType = ITYPE_ITEM,
		.maxUses = 5,
		.encodedRange = 0x11,
		.costPerUse = 300,
		.iconId = 0x67,
		.useEffectId = 38,
	},
	[ITEM_MINE] = {
		.nameTextId = 0x403,
		.descTextId = 0x4ab,
		.number = ITEM_MINE,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 500,
		.iconId = 0x8c,
		.useEffectId = 39,
	},
	[ITEM_LIGHTRUNE] = {
		.nameTextId = 0x403,
		.descTextId = 0x4ab,
		.number = ITEM_LIGHTRUNE,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 800,
		.iconId = 0x8d,
		.useEffectId = 40,
	},
	[ITEM_HOPLON_SHIELD] = {
		.nameTextId = 0x3f3,
		.descTextId = 0x4a4,
		.useDescTextId = 0x4dd,
		.number = ITEM_HOPLON_SHIELD,
		.weaponType = ITYPE_ITEM,
		.attributes = IA_UNBREAKABLE | IA_NEGATE_CRIT,
		.encodedRange = 0x11,
		.costPerUse = 5000,
		.iconId = 0xdb,
	},
	[ITEM_FILLAS_MIGHT] = {
		.nameTextId = 0x403,
		.descTextId = 0x4ab,
		.number = ITEM_FILLAS_MIGHT,
		.weaponType = ITYPE_12,
		.attributes = IA_HAMMERNE,
		.maxUses = 15,
		.encodedRange = 0x11,
		.iconId = 0x8f,
		.useEffectId = 41,
	},
	[ITEM_NINISS_GRACE] = {
		.nameTextId = 0x403,
		.descTextId = 0x4ab,
		.number = ITEM_NINISS_GRACE,
		.weaponType = ITYPE_12,
		.attributes = IA_HAMMERNE,
		.maxUses = 15,
		.encodedRange = 0x11,
		.iconId = 0x90,
		.useEffectId = 42,
	},
	[ITEM_THORS_IRE] = {
		.nameTextId = 0x403,
		.descTextId = 0x4ab,
		.number = ITEM_THORS_IRE,
		.weaponType = ITYPE_12,
		.attributes = IA_HAMMERNE,
		.maxUses = 15,
		.encodedRange = 0x11,
		.iconId = 0x91,
		.useEffectId = 43,
	},
	[ITEM_SETS_LITANY] = {
		.nameTextId = 0x403,
		.descTextId = 0x4ab,
		.number = ITEM_SETS_LITANY,
		.weaponType = ITYPE_12,
		.attributes = IA_HAMMERNE,
		.maxUses = 15,
		.encodedRange = 0x11,
		.iconId = 0x92,
		.useEffectId = 44,
	},
	[ITEM_SWORD_SHADOWKILLR] = {
		.nameTextId = 0x3a4,
		.descTextId = 0x454,
		.number = ITEM_SWORD_SHADOWKILLR,
		.weaponType = ITYPE_SWORD,
		.attributes = IA_WEAPON,
		.pEffectiveness = ItemEffectiveness_088ADF39,
		.maxUses = 60,
		.might = 5,
		.hit = 90,
		.weight = 5,
		.crit = 5,
		.encodedRange = 0x11,
		.weaponRank = WPN_EXP_E,
		.iconId = 0x93,
		.weaponExp = 1,
	},
	[ITEM_LANCE_BRIGHTLANCE] = {
		.nameTextId = 0x3a5,
		.descTextId = 0x455,
		.number = ITEM_LANCE_BRIGHTLANCE,
		.weaponType = ITYPE_LANCE,
		.attributes = IA_WEAPON,
		.pEffectiveness = ItemEffectiveness_088ADF39,
		.maxUses = 60,
		.might = 7,
		.hit = 80,
		.weight = 8,
		.crit = 5,
		.encodedRange = 0x11,
		.weaponRank = WPN_EXP_E,
		.iconId = 0x94,
		.weaponExp = 1,
	},
	[ITEM_AXE_FIENDCLEAVER] = {
		.nameTextId = 0x3a6,
		.descTextId = 0x456,
		.number = ITEM_AXE_FIENDCLEAVER,
		.weaponType = ITYPE_AXE,
		.attributes = IA_WEAPON,
		.pEffectiveness = ItemEffectiveness_088ADF39,
		.maxUses = 60,
		.might = 8,
		.hit = 75,
		.weight = 10,
		.crit = 5,
		.encodedRange = 0x11,
		.weaponRank = WPN_EXP_E,
		.iconId = 0x95,
		.weaponExp = 1,
	},
	[ITEM_BOW_BEACONBOW] = {
		.nameTextId = 0x3a7,
		.descTextId = 0x457,
		.number = ITEM_BOW_BEACONBOW,
		.weaponType = ITYPE_BOW,
		.attributes = IA_WEAPON,
		.pEffectiveness = ItemEffectiveness_088ADEF1,
		.maxUses = 60,
		.might = 6,
		.hit = 85,
		.weight = 5,
		.crit = 5,
		.encodedRange = 0x22,
		.weaponRank = WPN_EXP_E,
		.iconId = 0x96,
		.weaponExp = 1,
	},
	[ITEM_SWORD_SIEGLINDE] = {
		.nameTextId = 0x366,
		.descTextId = 0x416,
		.number = ITEM_SWORD_SIEGLINDE,
		.weaponType = ITYPE_SWORD,
		.attributes = IA_WEAPON | IA_UNSELLABLE | IA_LOCK_4,
		.pStatBonuses = (void *)0x088AEF48,
		.pEffectiveness = ItemEffectiveness_088ADF39,
		.maxUses = 30,
		.might = 16,
		.hit = 90,
		.weight = 9,
		.encodedRange = 0x11,
		.iconId = 0xb0,
		.weaponExp = 1,
	},
	[ITEM_AXE_BATTLEAXE] = {
		.nameTextId = 0x388,
		.descTextId = 0x439,
		.number = ITEM_AXE_BATTLEAXE,
		.weaponType = ITYPE_AXE,
		.attributes = IA_WEAPON,
		.maxUses = 20,
		.might = 13,
		.hit = 60,
		.weight = 15,
		.crit = 5,
		.encodedRange = 0x11,
		.costPerUse = 50,
		.weaponRank = WPN_EXP_B,
		.iconId = 0xb8,
		.weaponExp = 1,
	},
	[ITEM_LIGHT_IVALDI] = {
		.nameTextId = 0x3b5,
		.descTextId = 0x464,
		.number = ITEM_LIGHT_IVALDI,
		.weaponType = ITYPE_LIGHT,
		.attributes = IA_WEAPON | IA_MAGIC | IA_UNSELLABLE,
		.pStatBonuses = (void *)0x088AEF54,
		.pEffectiveness = ItemEffectiveness_088ADF39,
		.maxUses = 30,
		.might = 17,
		.hit = 90,
		.weight = 10,
		.crit = 5,
		.encodedRange = 0x12,
		.weaponRank = WPN_EXP_S,
		.iconId = 0xbb,
		.weaponExp = 1,
	},
	[ITEM_MASTERSEAL] = {
		.nameTextId = 0x3f4,
		.descTextId = 0x4a5,
		.useDescTextId = 0x4de,
		.number = ITEM_MASTERSEAL,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 20000,
		.iconId = 0xd9,
		.useEffectId = 45,
	},
	[ITEM_METISSTOME] = {
		.nameTextId = 0x3f5,
		.descTextId = 0x4a6,
		.useDescTextId = 0x4df,
		.number = ITEM_METISSTOME,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.iconId = 0xdc,
		.useEffectId = 46,
	},
	[ITEM_HEAVENSEAL] = {
		.nameTextId = 0x403,
		.descTextId = 0x4ab,
		.number = ITEM_HEAVENSEAL,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 20000,
		.iconId = 0x9c,
		.useEffectId = 47,
	},
	[ITEM_MONSTER_SHARPCLAW] = {
		.nameTextId = 0x39d,
		.descTextId = 0x44d,
		.number = ITEM_MONSTER_SHARPCLAW,
		.weaponType = ITYPE_11,
		.attributes = IA_WEAPON | IA_UNBREAKABLE | IA_LOCK_3,
		.might = 14,
		.hit = 65,
		.weight = 14,
		.encodedRange = 0x11,
		.iconId = 0xd0,
	},
	[ITEM_STAFF_LATONA] = {
		.nameTextId = 0x3d1,
		.descTextId = 0x480,
		.useDescTextId = 0x4be,
		.number = ITEM_STAFF_LATONA,
		.weaponType = ITYPE_STAFF,
		.attributes = IA_STAFF | IA_UNSELLABLE,
		.maxUses = 3,
		.hit = 100,
		.weight = 5,
		.encodedRange = 0xFF,
		.weaponRank = WPN_EXP_S,
		.iconId = 0xc3,
		.weaponExp = 1,
	},
	[ITEM_LANCE_DRAGON] = {
		.nameTextId = 0x377,
		.descTextId = 0x427,
		.number = ITEM_LANCE_DRAGON,
		.weaponType = ITYPE_LANCE,
		.attributes = IA_WEAPON,
		.pEffectiveness = ItemEffectiveness_088ADF13,
		.maxUses = 30,
		.might = 10,
		.hit = 70,
		.weight = 8,
		.encodedRange = 0x11,
		.costPerUse = 150,
		.weaponRank = WPN_EXP_C,
		.iconId = 0xb6,
		.weaponExp = 1,
	},
	[ITEM_LANCE_VIDOFNIR] = {
		.nameTextId = 0x376,
		.descTextId = 0x426,
		.number = ITEM_LANCE_VIDOFNIR,
		.weaponType = ITYPE_LANCE,
		.attributes = IA_WEAPON | IA_UNSELLABLE,
		.pStatBonuses = (void *)0x088AEF60,
		.pEffectiveness = ItemEffectiveness_088ADF39,
		.maxUses = 30,
		.might = 15,
		.hit = 85,
		.weight = 11,
		.encodedRange = 0x11,
		.weaponRank = WPN_EXP_S,
		.iconId = 0xb5,
		.weaponExp = 1,
	},
	[ITEM_DARK_NAGLFAR] = {
		.nameTextId = 0x3bc,
		.descTextId = 0x46b,
		.number = ITEM_DARK_NAGLFAR,
		.weaponType = ITYPE_DARK,
		.attributes = IA_WEAPON | IA_MAGIC | IA_UNBREAKABLE,
		.might = 25,
		.hit = 95,
		.weight = 18,
		.encodedRange = 0x12,
		.weaponRank = WPN_EXP_S,
		.iconId = 0xbd,
		.weaponExp = 1,
	},
	[ITEM_MONSTER_WRETCHAIR] = {
		.nameTextId = 0x397,
		.descTextId = 0x447,
		.useDescTextId = 0x4ac,
		.number = ITEM_MONSTER_WRETCHAIR,
		.weaponType = ITYPE_11,
		.attributes = IA_WEAPON | IA_UNBREAKABLE | IA_LOCK_3 | IA_NEGATE_DEFENSE,
		.pStatBonuses = (void *)0x088AEF6C,
		.might = 10,
		.hit = 100,
		.encodedRange = 0x12,
		.weaponRank = WPN_EXP_A,
		.iconId = 0x79,
	},
	[ITEM_SWORD_AUDHULMA] = {
		.nameTextId = 0x367,
		.descTextId = 0x417,
		.number = ITEM_SWORD_AUDHULMA,
		.weaponType = ITYPE_SWORD,
		.attributes = IA_WEAPON | IA_UNSELLABLE,
		.pStatBonuses = (void *)0x088AEF78,
		.pEffectiveness = ItemEffectiveness_088ADF39,
		.maxUses = 30,
		.might = 18,
		.hit = 85,
		.weight = 9,
		.encodedRange = 0x11,
		.weaponRank = WPN_EXP_S,
		.iconId = 0xb1,
		.weaponExp = 1,
	},
	[ITEM_LANCE_SIEGMUND] = {
		.nameTextId = 0x375,
		.descTextId = 0x425,
		.number = ITEM_LANCE_SIEGMUND,
		.weaponType = ITYPE_LANCE,
		.attributes = IA_WEAPON | IA_UNSELLABLE | IA_LOCK_5,
		.pStatBonuses = (void *)0x088AEF84,
		.pEffectiveness = ItemEffectiveness_088ADF39,
		.maxUses = 30,
		.might = 17,
		.hit = 80,
		.weight = 11,
		.encodedRange = 0x11,
		.iconId = 0xb4,
		.weaponExp = 1,
	},
	[ITEM_AXE_GARM] = {
		.nameTextId = 0x389,
		.descTextId = 0x438,
		.number = ITEM_AXE_GARM,
		.weaponType = ITYPE_AXE,
		.attributes = IA_WEAPON | IA_UNSELLABLE,
		.pStatBonuses = (void *)0x088AEF90,
		.pEffectiveness = ItemEffectiveness_088ADF39,
		.maxUses = 30,
		.might = 20,
		.hit = 75,
		.weight = 13,
		.encodedRange = 0x11,
		.weaponRank = WPN_EXP_S,
		.iconId = 0xb7,
		.weaponExp = 1,
	},
	[ITEM_BOW_NIDHOGG] = {
		.nameTextId = 0x396,
		.descTextId = 0x446,
		.number = ITEM_BOW_NIDHOGG,
		.weaponType = ITYPE_BOW,
		.attributes = IA_WEAPON | IA_UNSELLABLE,
		.pStatBonuses = (void *)0x088AEF9C,
		.pEffectiveness = ItemEffectiveness_088ADEF1,
		.maxUses = 30,
		.might = 18,
		.hit = 80,
		.weight = 7,
		.encodedRange = 0x22,
		.weaponRank = WPN_EXP_S,
		.iconId = 0xba,
		.weaponExp = 1,
	},
	[ITEM_LANCE_HEAVYSPEAR] = {
		.nameTextId = 0x378,
		.descTextId = 0x428,
		.number = ITEM_LANCE_HEAVYSPEAR,
		.weaponType = ITYPE_LANCE,
		.attributes = IA_WEAPON,
		.pEffectiveness = ItemEffectiveness_088ADEBB,
		.maxUses = 16,
		.might = 9,
		.hit = 70,
		.weight = 14,
		.encodedRange = 0x11,
		.costPerUse = 75,
		.weaponRank = WPN_EXP_D,
		.iconId = 0xa7,
		.weaponExp = 1,
	},
	[ITEM_LANCE_SHORTSPEAR] = {
		.nameTextId = 0x379,
		.descTextId = 0x429,
		.number = ITEM_LANCE_SHORTSPEAR,
		.weaponType = ITYPE_LANCE,
		.attributes = IA_WEAPON,
		.maxUses = 18,
		.might = 9,
		.hit = 60,
		.weight = 12,
		.encodedRange = 0x12,
		.costPerUse = 50,
		.weaponRank = WPN_EXP_C,
		.iconId = 0xa8,
		.weaponExp = 1,
	},
	[ITEM_OCEANSEAL] = {
		.nameTextId = 0x3f6,
		.descTextId = 0x4a7,
		.useDescTextId = 0x4e0,
		.number = ITEM_OCEANSEAL,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 10000,
		.iconId = 0xa9,
		.useEffectId = 48,
	},
	[ITEM_LUNARBRACE] = {
		.nameTextId = 0x3f7,
		.descTextId = 0x4a8,
		.useDescTextId = 0x4e1,
		.number = ITEM_LUNARBRACE,
		.weaponType = ITYPE_ITEM,
		.attributes = IA_UNSELLABLE,
		.maxUses = 1,
		.encodedRange = 0x11,
		.iconId = 0xd8,
		.useEffectId = 49,
	},
	[ITEM_SOLARBRACE] = {
		.nameTextId = 0x3f8,
		.descTextId = 0x4a9,
		.useDescTextId = 0x4e2,
		.number = ITEM_SOLARBRACE,
		.weaponType = ITYPE_ITEM,
		.attributes = IA_UNSELLABLE,
		.maxUses = 1,
		.encodedRange = 0x11,
		.iconId = 0xd7,
		.useEffectId = 50,
	},
	[ITEM_1G] = {
		.nameTextId = 0x3f9,
		.descTextId = 0x4a1,
		.useDescTextId = 0x4da,
		.number = ITEM_1G,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 1,
		.iconId = 0x89,
	},
	[ITEM_5G] = {
		.nameTextId = 0x3fa,
		.descTextId = 0x4a1,
		.useDescTextId = 0x4da,
		.number = ITEM_5G,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 5,
		.iconId = 0x89,
	},
	[ITEM_10G] = {
		.nameTextId = 0x3fb,
		.descTextId = 0x4a1,
		.useDescTextId = 0x4da,
		.number = ITEM_10G,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 10,
		.iconId = 0x89,
	},
	[ITEM_50G] = {
		.nameTextId = 0x3fc,
		.descTextId = 0x4a1,
		.useDescTextId = 0x4da,
		.number = ITEM_50G,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 50,
		.iconId = 0x89,
	},
	[ITEM_100G] = {
		.nameTextId = 0x3fd,
		.descTextId = 0x4a1,
		.useDescTextId = 0x4da,
		.number = ITEM_100G,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 100,
		.iconId = 0x89,
	},
	[ITEM_3000G] = {
		.nameTextId = 0x400,
		.descTextId = 0x4a1,
		.useDescTextId = 0x4da,
		.number = ITEM_3000G,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 3000,
		.iconId = 0x89,
	},
	[ITEM_5000G] = {
		.nameTextId = 0x401,
		.descTextId = 0x4a1,
		.useDescTextId = 0x4da,
		.number = ITEM_5000G,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 5000,
		.iconId = 0x89,
	},
	[ITEM_SWORD_WINDSWORD] = {
		.nameTextId = 0x368,
		.descTextId = 0x418,
		.number = ITEM_SWORD_WINDSWORD,
		.weaponType = ITYPE_SWORD,
		.attributes = IA_WEAPON | IA_MAGICDAMAGE,
		.pEffectiveness = ItemEffectiveness_088ADF2A,
		.maxUses = 40,
		.might = 9,
		.hit = 70,
		.weight = 9,
		.encodedRange = 0x12,
		.costPerUse = 200,
		.weaponRank = WPN_EXP_B,
		.iconId = 0xaa,
		.weaponExp = 1,
	},
	[ITEM_VULNERARY_2] = {
		.nameTextId = 0x3e5,
		.descTextId = 0x494,
		.useDescTextId = 0x4d2,
		.number = ITEM_VULNERARY_2,
		.weaponType = ITYPE_ITEM,
		.maxUses = 60,
		.encodedRange = 0x11,
		.costPerUse = 100,
		.iconId = 0x6a,
		.useEffectId = 51,
	},
	[ITEM_UNK_GREENNOTE] = {
		.nameTextId = 0x3e5,
		.descTextId = 0x494,
		.useDescTextId = 0x4d2,
		.number = ITEM_UNK_GREENNOTE,
		.weaponType = ITYPE_ITEM,
		.maxUses = 60,
		.encodedRange = 0x11,
		.costPerUse = 100,
		.iconId = 0xab,
		.useEffectId = 52,
	},
	[ITEM_UNK_REDNOTE] = {
		.nameTextId = 0x3e5,
		.descTextId = 0x494,
		.useDescTextId = 0x4d2,
		.number = ITEM_UNK_REDNOTE,
		.weaponType = ITYPE_ITEM,
		.maxUses = 60,
		.encodedRange = 0x11,
		.costPerUse = 100,
		.iconId = 0xac,
		.useEffectId = 53,
	},
	[ITEM_DANCE] = {
		.nameTextId = 0x3d2,
		.descTextId = 0x481,
		.useDescTextId = 0x4bf,
		.number = ITEM_DANCE,
		.weaponType = ITYPE_12,
		.encodedRange = 0x11,
		.iconId = 0x0,
	},
	[ITEM_NIGHTMARE] = {
		.nameTextId = 0x39c,
		.descTextId = 0x44c,
		.number = ITEM_NIGHTMARE,
		.weaponType = ITYPE_STAFF,
		.attributes = IA_STAFF,
		.pStatBonuses = (void *)0x088AEFA8,
		.maxUses = 1,
		.hit = 70,
		.weight = 8,
		.encodedRange = 0x13,
		.iconId = 0xc7,
	},
	[ITEM_DEMONSTONE] = {
		.nameTextId = 0x399,
		.descTextId = 0x449,
		.useDescTextId = 0x4ae,
		.number = ITEM_DEMONSTONE,
		.weaponType = ITYPE_11,
		.attributes = IA_WEAPON | IA_UNBREAKABLE | IA_LOCK_3,
		.might = 10,
		.hit = 65,
		.encodedRange = 0x11,
		.iconId = 0x88,
	},
	[ITEM_DEMONLIGHT] = {
		.nameTextId = 0x39a,
		.descTextId = 0x44a,
		.number = ITEM_DEMONLIGHT,
		.weaponType = ITYPE_11,
		.attributes = IA_WEAPON | IA_UNBREAKABLE | IA_LOCK_3,
		.pStatBonuses = (void *)0x088AEFB4,
		.might = 15,
		.hit = 60,
		.encodedRange = 0x13,
		.iconId = 0xc5,
	},
	[ITEM_RAVAGER] = {
		.nameTextId = 0x39b,
		.descTextId = 0x44b,
		.number = ITEM_RAVAGER,
		.weaponType = ITYPE_11,
		.attributes = IA_WEAPON | IA_UNBREAKABLE | IA_LOCK_3,
		.pStatBonuses = (void *)0x088AEFC0,
		.might = 15,
		.hit = 85,
		.crit = 10,
		.encodedRange = 0x11,
		.iconId = 0xc6,
	},
	[ITEM_DIVINESTONE] = {
		.nameTextId = 0x398,
		.descTextId = 0x448,
		.useDescTextId = 0x4ad,
		.number = ITEM_DIVINESTONE,
		.weaponType = ITYPE_11,
		.attributes = IA_WEAPON | IA_UNSELLABLE | IA_LOCK_3,
		.pStatBonuses = (void *)0x088AEFCC,
		.pEffectiveness = ItemEffectiveness_088ADF39,
		.maxUses = 50,
		.might = 16,
		.hit = 100,
		.crit = 15,
		.encodedRange = 0x11,
		.iconId = 0xc4,
	},
	[ITEM_MONSTER_DEMONSURG] = {
		.nameTextId = 0x3c0,
		.descTextId = 0x46f,
		.number = ITEM_MONSTER_DEMONSURG,
		.weaponType = ITYPE_DARK,
		.attributes = IA_WEAPON | IA_MAGIC | IA_UNBREAKABLE,
		.might = 11,
		.hit = 80,
		.weight = 10,
		.encodedRange = 0x12,
		.iconId = 0xc1,
	},
	[ITEM_MONSTER_SHADOWSHT] = {
		.nameTextId = 0x3c1,
		.descTextId = 0x470,
		.number = ITEM_MONSTER_SHADOWSHT,
		.weaponType = ITYPE_DARK,
		.attributes = IA_WEAPON | IA_MAGIC | IA_UNCOUNTERABLE,
		.maxUses = 5,
		.might = 13,
		.hit = 70,
		.weight = 20,
		.crit = 5,
		.encodedRange = 0x3A,
		.iconId = 0xc2,
	},
	[ITEM_MONSTER_ROTTENCLW] = {
		.nameTextId = 0x39e,
		.descTextId = 0x44e,
		.number = ITEM_MONSTER_ROTTENCLW,
		.weaponType = ITYPE_11,
		.attributes = IA_WEAPON | IA_UNBREAKABLE | IA_LOCK_3,
		.might = 7,
		.hit = 80,
		.weight = 8,
		.encodedRange = 0x11,
		.iconId = 0xd1,
	},
	[ITEM_MONSTER_FETIDCLW] = {
		.nameTextId = 0x39f,
		.descTextId = 0x44f,
		.number = ITEM_MONSTER_FETIDCLW,
		.weaponType = ITYPE_11,
		.attributes = IA_WEAPON | IA_UNBREAKABLE | IA_LOCK_3,
		.might = 12,
		.hit = 75,
		.weight = 10,
		.encodedRange = 0x11,
		.iconId = 0xd2,
	},
	[ITEM_MONSTER_VENINCLW] = {
		.nameTextId = 0x3a0,
		.descTextId = 0x450,
		.number = ITEM_MONSTER_VENINCLW,
		.weaponType = ITYPE_11,
		.attributes = IA_WEAPON | IA_UNBREAKABLE | IA_LOCK_3,
		.might = 6,
		.hit = 65,
		.weight = 10,
		.encodedRange = 0x11,
		.iconId = 0xd3,
		.weaponEffectId = WPN_EFFECT_POISON,
	},
	[ITEM_MONSTER_LTHLTALON] = {
		.nameTextId = 0x3a1,
		.descTextId = 0x451,
		.number = ITEM_MONSTER_LTHLTALON,
		.weaponType = ITYPE_11,
		.attributes = IA_WEAPON | IA_UNBREAKABLE | IA_LOCK_3,
		.might = 10,
		.hit = 60,
		.weight = 12,
		.encodedRange = 0x11,
		.iconId = 0xd4,
		.weaponEffectId = WPN_EFFECT_POISON,
	},
	[ITEM_MONSTER_FIREFANG] = {
		.nameTextId = 0x3a2,
		.descTextId = 0x452,
		.number = ITEM_MONSTER_FIREFANG,
		.weaponType = ITYPE_11,
		.attributes = IA_WEAPON | IA_UNBREAKABLE | IA_LOCK_3,
		.might = 5,
		.hit = 90,
		.weight = 5,
		.encodedRange = 0x11,
		.iconId = 0xd5,
	},
	[ITEM_MONSTER_HELLFANG] = {
		.nameTextId = 0x3a3,
		.descTextId = 0x453,
		.number = ITEM_MONSTER_HELLFANG,
		.weaponType = ITYPE_11,
		.attributes = IA_WEAPON | IA_UNBREAKABLE | IA_LOCK_3,
		.might = 13,
		.hit = 80,
		.weight = 8,
		.encodedRange = 0x11,
		.iconId = 0xd6,
	},
	[ITEM_MONSTER_EVILEYE] = {
		.nameTextId = 0x3be,
		.descTextId = 0x46d,
		.number = ITEM_MONSTER_EVILEYE,
		.weaponType = ITYPE_DARK,
		.attributes = IA_WEAPON | IA_MAGIC | IA_UNBREAKABLE,
		.might = 7,
		.hit = 85,
		.weight = 6,
		.encodedRange = 0x12,
		.iconId = 0xbe,
	},
	[ITEM_MONSTER_CRIMSNEYE] = {
		.nameTextId = 0x3bf,
		.descTextId = 0x46e,
		.number = ITEM_MONSTER_CRIMSNEYE,
		.weaponType = ITYPE_DARK,
		.attributes = IA_WEAPON | IA_MAGIC | IA_UNBREAKABLE,
		.might = 14,
		.hit = 70,
		.weight = 13,
		.crit = 5,
		.encodedRange = 0x12,
		.iconId = 0xbf,
	},
	[ITEM_MONSTER_STONE] = {
		.nameTextId = 0x3bd,
		.descTextId = 0x46c,
		.number = ITEM_MONSTER_STONE,
		.weaponType = ITYPE_DARK,
		.attributes = IA_WEAPON | IA_MAGIC,
		.maxUses = 5,
		.hit = 65,
		.weight = 8,
		.encodedRange = 0x13,
		.iconId = 0xc0,
		.weaponEffectId = WPN_EFFECT_PETRIFY,
		.weaponExp = 5,
	},
	[ITEM_ANIMA_AIRCALIBUR] = {
		.nameTextId = 0x3ac,
		.descTextId = 0x45b,
		.number = ITEM_ANIMA_AIRCALIBUR,
		.weaponType = ITYPE_ANIMA,
		.attributes = IA_WEAPON | IA_MAGIC,
		.pEffectiveness = ItemEffectiveness_088ADF2A,
		.maxUses = 20,
		.might = 8,
		.hit = 85,
		.weight = 2,
		.encodedRange = 0x12,
		.costPerUse = 55,
		.weaponRank = WPN_EXP_B,
		.iconId = 0x35,
		.weaponExp = 1,
	},
	[ITEM_JUNAFRUIT] = {
		.nameTextId = 0x402,
		.descTextId = 0x4aa,
		.useDescTextId = 0x4e3,
		.number = ITEM_JUNAFRUIT,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.iconId = 0xdd,
		.useEffectId = 54,
	},
	[ITEM_150G] = {
		.nameTextId = 0x3fe,
		.descTextId = 0x4a1,
		.useDescTextId = 0x4da,
		.number = ITEM_150G,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 150,
		.iconId = 0x89,
	},
	[ITEM_200G] = {
		.nameTextId = 0x3ff,
		.descTextId = 0x4a1,
		.useDescTextId = 0x4da,
		.number = ITEM_200G,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 200,
		.iconId = 0x89,
	},
	[ITEM_BLACKGEM] = {
		.nameTextId = 0x3ee,
		.descTextId = 0x49d,
		.number = ITEM_BLACKGEM,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 30000,
		.iconId = 0xc8,
	},
	[ITEM_GOLDGEM] = {
		.nameTextId = 0x3ed,
		.descTextId = 0x49c,
		.number = ITEM_GOLDGEM,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.costPerUse = 40000,
		.iconId = 0xc9,
	},
	[ITEM_UNK_BC] = {
		.nameTextId = 0x403,
		.descTextId = 0x4ab,
		.number = ITEM_UNK_BC,
		.weaponType = ITYPE_SWORD,
		.attributes = IA_WEAPON,
		.encodedRange = 0x11,
		.iconId = 0x0,
		.weaponExp = 1,
	},
	[ITEM_UNK_BD] = {
		.nameTextId = 0x403,
		.descTextId = 0x4ab,
		.number = ITEM_UNK_BD,
		.weaponType = ITYPE_AXE,
		.attributes = IA_WEAPON,
		.encodedRange = 0x11,
		.iconId = 0x0,
		.weaponExp = 1,
	},
	[ITEM_UNK_BE] = {
		.nameTextId = 0x403,
		.descTextId = 0x4ab,
		.number = ITEM_UNK_BE,
		.weaponType = ITYPE_DARK,
		.attributes = IA_WEAPON | IA_MAGIC,
		.might = 11,
		.hit = 80,
		.weight = 10,
		.encodedRange = 0x12,
		.iconId = 0x0,
		.weaponExp = 1,
	},
	[ITEM_UNK_BF] = {
		.nameTextId = 0x403,
		.descTextId = 0x4ab,
		.number = ITEM_UNK_BF,
		.weaponType = ITYPE_DARK,
		.attributes = IA_WEAPON | IA_MAGIC,
		.might = 11,
		.hit = 80,
		.weight = 10,
		.encodedRange = 0x12,
		.iconId = 0x0,
		.weaponExp = 1,
	},
	[ITEM_UNK_C0] = {
		.nameTextId = 0x403,
		.descTextId = 0x4ab,
		.number = ITEM_UNK_C0,
		.weaponType = ITYPE_SWORD,
		.attributes = IA_WEAPON,
		.encodedRange = 0x11,
		.iconId = 0x0,
		.weaponExp = 1,
	},
	[ITEM_UNK_C1] = {
		.nameTextId = 0x403,
		.descTextId = 0x4ab,
		.number = ITEM_UNK_C1,
		.weaponType = ITYPE_ITEM,
		.encodedRange = 0x11,
		.iconId = 0x0,
		.weaponExp = 1,
	},
	[ITEM_UNK_C2] = {
		.nameTextId = 0x403,
		.descTextId = 0x4ab,
		.number = ITEM_UNK_C2,
		.weaponType = ITYPE_SWORD,
		.attributes = IA_WEAPON,
		.encodedRange = 0x11,
		.iconId = 0x0,
		.weaponExp = 1,
	},
	[ITEM_UNK_C3] = {
		.nameTextId = 0x403,
		.descTextId = 0x4ab,
		.number = ITEM_UNK_C3,
		.weaponType = ITYPE_LANCE,
		.attributes = IA_WEAPON,
		.encodedRange = 0x11,
		.iconId = 0x0,
		.weaponExp = 1,
	},
	[ITEM_UNK_C4] = {
		.nameTextId = 0x403,
		.descTextId = 0x4ab,
		.number = ITEM_UNK_C4,
		.weaponType = ITYPE_AXE,
		.attributes = IA_WEAPON,
		.might = 11,
		.hit = 80,
		.weight = 10,
		.encodedRange = 0x11,
		.iconId = 0x0,
		.weaponExp = 1,
	},
	[ITEM_UNK_C5] = {
		.nameTextId = 0x403,
		.descTextId = 0x4ab,
		.number = ITEM_UNK_C5,
		.weaponType = ITYPE_AXE,
		.attributes = IA_WEAPON,
		.might = 11,
		.hit = 80,
		.weight = 10,
		.encodedRange = 0x11,
		.iconId = 0x0,
		.weaponExp = 1,
	},
	[ITEM_UNK_C6] = {
		.nameTextId = 0x403,
		.descTextId = 0x4ab,
		.number = ITEM_UNK_C6,
		.weaponType = ITYPE_SWORD,
		.attributes = IA_WEAPON,
		.encodedRange = 0x11,
		.iconId = 0x0,
		.weaponExp = 1,
	},
	[ITEM_UNK_C7] = {
		.nameTextId = 0x403,
		.descTextId = 0x4ab,
		.number = ITEM_UNK_C7,
		.weaponType = ITYPE_BOW,
		.attributes = IA_WEAPON,
		.might = 11,
		.hit = 80,
		.weight = 10,
		.encodedRange = 0x11,
		.iconId = 0x0,
		.weaponExp = 1,
	},
	[ITEM_UNK_C8] = {
		.nameTextId = 0x403,
		.descTextId = 0x4ab,
		.number = ITEM_UNK_C8,
		.weaponType = ITYPE_LANCE,
		.attributes = IA_WEAPON,
		.encodedRange = 0x11,
		.iconId = 0x0,
		.weaponExp = 1,
	},
	[ITEM_UNK_C9] = {
		.nameTextId = 0x403,
		.descTextId = 0x4ab,
		.number = ITEM_UNK_C9,
		.weaponType = ITYPE_LIGHT,
		.attributes = IA_WEAPON | IA_MAGIC,
		.might = 11,
		.hit = 80,
		.weight = 10,
		.encodedRange = 0x12,
		.iconId = 0x0,
		.weaponExp = 1,
	},
	[ITEM_UNK_CA] = {
		.nameTextId = 0x403,
		.descTextId = 0x4ab,
		.number = ITEM_UNK_CA,
		.weaponType = ITYPE_DARK,
		.attributes = IA_WEAPON | IA_MAGIC,
		.encodedRange = 0x12,
		.iconId = 0x0,
		.weaponExp = 1,
	},
	[ITEM_UNK_CB] = {
		.nameTextId = 0x403,
		.descTextId = 0x4ab,
		.number = ITEM_UNK_CB,
		.weaponType = ITYPE_DARK,
		.attributes = IA_WEAPON | IA_MAGIC,
		.encodedRange = 0x12,
		.iconId = 0x0,
		.weaponExp = 1,
	},
	[ITEM_UNK_CC] = {
		.nameTextId = 0x403,
		.descTextId = 0x4ab,
		.number = ITEM_UNK_CC,
		.weaponType = ITYPE_ITEM,
		.maxUses = 1,
		.encodedRange = 0x11,
		.iconId = 0x9d,
	},
	[ITEM_UNK_CD] = {
		.nameTextId = 0x403,
		.descTextId = 0x4ab,
		.number = ITEM_UNK_CD,
		.weaponType = ITYPE_12,
		.encodedRange = 0x12,
		.iconId = 0x0,
		.weaponExp = 1,
	},
};
